...
### System costs per MWh electricity

This includes all electricity for final energy and for hydrogen production.

```{python system costs per mwh, echo=FALSE, warning = FALSE, message = FALSE}
...
for iteration in df['iteration'].unique():
    ...
    for _, row in df[df['iteration'] == iteration].iterrows():
        ...
        # Get electricity load + electricity for additional hydrogen production
        elec_load = n.statistics.energy_balance(comps="Load", bus_carrier="AC")
        h2_load = n.statistics.energy_balance(comps="Load", bus_carrier="H2 demand REMIND")

        # Get efficiency of electrolysis link
        elec_eff = n.links.loc[n.links.carrier == "H2 electrolysis", "efficiency"].unique()[0]
        elec_load_h2 = h2_load / elec_eff
        total_elec_load = -(elec_load.values + elec_load_h2.values)[0]

        # Calculate system costs per MWh electricity
        system_cost_per_mwh = system_cost / total_elec_load

        # Show system costs
        ax = axs[counter // 5, counter % 5]
        system_cost_per_mwh.plot(ax=ax,
            kind="bar",
            stacked=True,
            color = [color_series[col] for col in system_cost.columns],
            legend=False,
        ...
        # Update global max for y-axis
        global_max = max(global_max, system_cost_per_mwh.values.sum())

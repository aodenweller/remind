...
library(lubridate)
library(reticulate)
use_condaenv(condaenv = "pypsa-eur-20241119", conda = "/p/system/packages/tools/anaconda/2024.10/bin/conda")
...
knitr::opts_chunk$set(
  ...
  cache = FALSE,
  warning = FALSE,
  error = TRUE, # Continue despite errors
  dev = c("png", "pdf"),
...
pyResultsDirs <- list.dirs(pyResultsDir,
                           recursive = FALSE,
                           full.names = TRUE)
# Get coupling_parameters subdirectories
pyCouplingDirs <- list.dirs(pyResultsDir,
                            recursive = TRUE,
                            full.names = TRUE) %>%
  keep(str_detect(., "coupling_parameters"))
pyReportingDirs <- list.dirs(pyResultsDir,
                             recursive = TRUE,
                             full.names = TRUE) %>%
...
pyResourcesIterDirs <- list.dirs(pyResourcesDir,
                                recursive = TRUE,
                                full.names = TRUE) %>%
  keep(str_detect(., "i\\d{1,}$"))
# Get iteration from PyPSA-Eur coupling_parameters directories
pyIterations <- as.numeric(gsub(".*i([0-9]+).*", "\\1", pyCouplingDirs)) %>%
  sort()
...
cat("Found", length(pyCouplingDirs), "PyPSA-EUR coupling_parameters directories.\n")
...
# Read REMIND electricity load data
dataLoadREMIND <- readREMINDgdx(
    paths = rm2pyFiles,
    gdxVar = "v32_load_sector",
    columns = c("ttot" = "year", "all_regi" = "region", "loadPy32" = "sector", "value" = "value"),
    colFilter = list("year" = years, "region" = regions),
    restoreZeros = FALSE,
    recalcUnit = twa2twh) %>%
  filter(iteration %in% pyIterations) %>%
  revalue.levels(sector = rm2genTech) %>%
  order.levels(sector = names(colorsTech))

dataLoadREMINDtot <- dataLoadREMIND %>%
...
dataLoadPyPSA <- readPyPSAcsv(
  ...
  filter(component %in% c("Load", "Link"),
         general_carrier %in% c("AC", "BEV pass charger", "BEV freight charger", "heatpump", "resistive")) %>%
  mutate(value = -value / 1e6)  # Convert MWh to TWh

dataLoadH2PyPSA <- readPyPSAcsv(
    paths = file.path(pyReportingDirs, "energy_balance_addH2.csv")) %>%
  filter(component == "Load") %>%
  mutate(value = -value / 1e6) %>%  # Convert MWh to TWh
  mutate(general_carrier = "electrolysis")

dataEtaPyPSA <- readPyPSAcsv(
    paths = file.path(pyResourcesDirs, "costs.csv")) %>%
  filter(technology == "electrolysis",
         parameter == "efficiency") %>%
  select(year, iteration, value) %>%
  rename(eta = value)

dataElectrolysisLoadPyPSA <- dataLoadH2PyPSA %>%
  left_join(dataEtaPyPSA, by = c("year", "iteration")) %>%
  mutate(value = value / eta) %>%  # Convert hydrogen demand to electricity load
  select(-eta) %>%
  mutate(general_carrier = "electrolysis")

dataLoadPyPSA <- dataLoadPyPSA %>%
  bind_rows(dataElectrolysisLoadPyPSA) %>%
  revalue.levels(general_carrier = py2genTech) %>%
  order.levels(general_carrier = names(colorsTech)) %>%
  rename(sector = general_carrier)

dataLoadPyPSAtot <- dataLoadPyPSA %>%
  group_by(year, region, iteration) %>%
  summarise(value = sum(value, na.rm = TRUE))

# Load total
pTotYear <- ggplot() + 
    geom_line(data = dataLoadREMINDtot, 
              aes(x = year, y = value, color = region)) +
    geom_point(data = dataLoadPyPSAtot,
               aes(x = year, y = value, color = region)) +
    scale_fill_discrete(name = "Region") +
    labs(x = "Year", y = "Electricity load (TWh/a)") +
    scale_y_continuous(limits = c(0, NA)) +
    ggtitle("Total electricity load by year") +
...
pTotIter <- ggplot() + 
    geom_line(data = dataLoadREMINDtot, 
              aes(x = iteration, y = value, color = region)) +
    geom_point(data = dataLoadPyPSAtot,
               aes(x = iteration, y = value, color = region)) +
    scale_fill_discrete(name = "Region") +
    ...
    scale_y_continuous(limits = c(0, NA)) +
    ggtitle("Total electricity load by iteration") +
    facet_wrap(~ year)
...
# By sector
pYear <- ggplot() + 
    geom_col(data = dataLoadREMIND, 
             aes(x = year, y = value, fill = sector), 
             position = position_stacknudge(x = -1),
             width = 1) +
    geom_col(data = dataLoadPyPSA,
             aes(x = year, y = value, fill = sector), 
             position = position_stacknudge(x = 1),
             width = 1) +
    scale_fill_manual(name = "Sector", values = colorsTech, labels = namesTech) +
    geom_line(data = dataLoadREMINDtot, 
              aes(x = year, y = value, linetype = "REMIND")) +
    geom_line(data = dataLoadPyPSAtot,
              aes(x = year, y = value, linetype = "PyPSA-Eur")) +
    scale_linetype_manual(name = NULL, values = c("REMIND" = "solid", "PyPSA-Eur" = "dashed")) +
    labs(x = "Year", y = "Electricity load (TWh/a)") +
    scale_y_continuous(limits = c(0, NA)) +
    ggtitle("Electricity load by sector and year") +
    facet_wrap(~ iteration)
...
pIter <- ggplot() + 
    geom_col(data = dataLoadREMIND, 
             aes(x = iteration, y = value, fill = sector), 
             position = position_stacknudge(x = -1),
             width = 0.5) +
    geom_col(data = dataLoadPyPSA,
             aes(x = iteration, y = value, fill = sector), 
    ...
    scale_fill_manual(name = "Sector", values = colorsTech, labels = namesTech) +
    geom_line(data = dataLoadREMINDtot, 
              aes(x = iteration, y = value, linetype = "REMIND")) +
    geom_line(data = dataLoadPyPSAtot,
              aes(x = iteration, y = value, linetype = "PyPSA-Eur")) +
    scale_linetype_manual(name = NULL, values = c("REMIND" = "solid", "PyPSA-Eur" = "dashed")) +
    labs(x = "Iteration", y = "Electricity load (TWh/a)") +
    scale_x_continuous(breaks = integer_breaks()) +
    scale_y_continuous(limits = c(0, NA)) +
    ggtitle("Electricity load by sector and iteration") +
    facet_wrap(~ year)

print(pIter)

# Sectoral load line charts
pYear <- ggplot() +
  geom_line(data = dataLoadREMIND,
            aes(x = year, y = value, color = sector)) +
  geom_point(data = dataLoadPyPSA,
             aes(x = year, y = value, color = sector)) +
  scale_color_manual(name = "Sector", values = colorsTech, labels = namesTech) +
  labs(x = "Year", y = "Electricity load (TWh/a)") +
  scale_y_continuous(limits = c(0, NA)) +
  ggtitle("Electricity load by sector and year") +
  facet_wrap(~ iteration)
...
pIter <- ggplot() +
  geom_line(data = dataLoadREMIND,
            aes(x = iteration, y = value, color = sector)) +
  geom_point(data = dataLoadPyPSA,
             aes(x = iteration, y = value, color = sector)) +
  scale_color_manual(name = "Sector", values = colorsTech, labels = namesTech) +
  labs(x = "Iteration", y = "Electricity load (TWh/a)") +
...
pYearTech <- pYear +
  facet_grid(sector ~ iteration, scales = "free")

print(pYearTech)

pIterTech <- pIter +
  facet_grid(sector ~ year, scales = "free")

print(pIterTech)

```

### Pre-investment capacities (generation and links)
...
# Average pre-investment capacities
dataPreInvCapAvgREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "p32_capAvg",
    columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "p32_cap"),
  ...
  filter(!(technology %in% storageTech)) %>%
  group_by(year, region, technology, iteration) %>%
  summarise(preInvCap = sum(p32_cap)) %>%
...
dataPreInvCapPyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "preinstalled_capacities.csv")) %>%
  filter(component %in% c("Generator", "Link")) %>%
  mutate(value = value / 1e3) %>%   # Convert capacities to GW
  rename(preInvCap = value) %>%
  rename(technology = general_carrier) %>%
  revalue.levels(technology = py2genTech)

# Special treatment for links: Pre-investment capacity is defined w.r.t. input, not output
# E.g., for electrolysis the capacity is defined w.r.t. electrical input
# Multiply with efficiency to get output capacity like in REMIND
dataEtaPyPSA <- readPyPSAcsv(
  paths = file.path(pyResourcesDirs, "costs.csv")) %>%
  ...
  rename(eta = value) %>%
  select(year, technology, iteration, eta)

# For link components, multiply with efficiency
dataPreInvCapPyPSA <- dataPreInvCapPyPSA %>%
  left_join(dataEtaPyPSA) %>%
  mutate(preInvCap = if_else(component == "Link",
                             preInvCap * eta,
                             preInvCap)) %>%
  order.levels(technology = names(colorsTech))
...
### Pre-investment capacities (stores)

```{r preinvestment capacities stores}
...
dataPreInvCapAvgREMIND <- readREMINDgdx(
    ...
    colFilter = list("year" = years, "region" = regions, "technology" = c("h2stor")),
    restoreZeros = TRUE) %>%
  filter(iteration %in% pyIterations)
...
dataPreInvCapPyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "preinstalled_capacities.csv")) %>%
  filter(general_carrier == "H2") %>%
  mutate(value = value / 1e6)  # Convert capacities to TWh

# Plot by year
pYear <- ggplot() +
  geom_line(data = dataPreInvCapAvgREMIND, aes(x = year, y = value, color = region)) +
  geom_point(data = dataPreInvCapREMIND, aes(x = year, y = value, color = region, shape = "Non-avg. value")) +
  geom_point(data = dataPreInvCapPyPSA, aes(x = year, y = value, color = region), alpha = 0.5) +
  scale_color_discrete(name = "Region") +
  scale_shape_manual(name = NULL, values = c("Non-avg. value" = 4)) +
  labs(x = "Year", y = "Capacity (TWh)") +
  ggtitle("Pre-investment hydrogen storage capacities over years") +
  facet_wrap(~ iteration)
...
# Plot by iteration
pIter <- ggplot() +
  geom_line(data = dataPreInvCapAvgREMIND, aes(x = iteration, y = value, color = region)) +
  geom_point(data = dataPreInvCapREMIND, aes(x = iteration, y = value, color = region, shape = "Non-avg. value")) +
  geom_point(data = dataPreInvCapPyPSA, aes(x = iteration, y = value, color = region), alpha = 0.5) +
  scale_color_discrete(name = "Region") +
  scale_shape_manual(name = NULL, values = c("Non-avg. value" = 4)) +
  labs(x = "Iteration", y = "Capacity (TWh)") +
  ggtitle("Pre-investment hydrogen storage capacities over iterations") +
  facet_wrap(~ year)
...
dataCapexWeightedREMIND <- dataCapexREMIND %>%
  ...
  order.levels(technology = names(colorsTech)) %>%
  filter(iteration %in% pyIterations,
         !(technology %in% storageTech))

# PyPSA-Eur
dataCapexPyPSA <- readPyPSAcsv(
  paths = file.path(pyResourcesDirs, "costs.csv")) %>%
  filter(parameter == "investment",
         !(technology %in% c("hydrogen storage underground", "battery storage")))

# Special treatment for electrolysers and battery inverters: Costs in costs.csv are per MW_in, not per MW_out
dataEtaPyPSA <- readPyPSAcsv(
  paths = file.path(pyResourcesDirs, "costs.csv")) %>%
  filter(parameter == "efficiency",
         technology %in% c("electrolysis", "battery inverter")) %>%
  revalue.levels(technology = py2genTech) %>%
  rename(eta = value) %>%
  select(technology, iteration, year, eta)
...
dataCapexPyPSArm <- dataCapexPyPSA %>%
  ...
  # Special treatment for electrolysers: Costs are per MWel not per MW
  full_join(dataEtaPyPSA) %>%
  mutate(value = if_else(technology %in% c("electrolysis", "battery inverter"), value * eta, value)) %>%
...
### Specific capital costs w/ adjustment costs (hydrogen underground storage)
...
dataCapexREMIND <- readREMINDgdx(
    ...
    gdxVar = "p32_capCostwAdjCost",
    columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "technology" = "h2stor"),
    restoreZeros = FALSE,
    recalcUnit = trUSDpTW2USDpMW) %>%
  filter(iteration %in% pyIterations) %>%
  revalue.levels(technology = rm2genTech)
...
dataCapexPyPSA <- readPyPSAcsv(
  ...
  revalue.levels(technology = c("hydrogen storage underground" = "hydrogen_storage"))

# Plot
pCapexYear <- ggplot() +
  geom_line(data = dataCapexREMIND, aes(x = year, y = value, color = technology)) +
  geom_point(data = dataCapexPyPSA, aes(x = year, y = value, color = technology), alpha = 0.5) +
  scale_color_manual(name = "Technology",
                     values = colorsTech,
                     labels = namesTech) +
  labs(x = "Year", y = "Specific capital costs (USD/MWh)") +
  scale_y_continuous(limits = c(0, NA)) +
  ggtitle("Specific capital costs (w/ adjustment costs) of hydrogen underground storage over years") +
  facet_wrap(~ iteration)
...
### Specific capital costs w/ adjustment costs (battery storage pack)
...
dataCapexREMIND <- readREMINDgdx(
    ...
    gdxVar = "p32_capCostwAdjCost",
    columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "technology" = "btstor"),
    restoreZeros = FALSE,
    recalcUnit = trUSDpTW2USDpMW) %>%
  filter(iteration %in% pyIterations) %>%
  revalue.levels(technology = rm2genTech)
...
dataCapexPyPSA <- readPyPSAcsv(
  ...
  revalue.levels(technology = c("battery storage" = "battery_storage"))

# Plot
pCapexYear <- ggplot() +
  geom_line(data = dataCapexREMIND, aes(x = year, y = value/1E3, color = technology)) +
  geom_point(data = dataCapexPyPSA, aes(x = year, y = value/1E3, color = technology), alpha = 0.5) +
  scale_color_manual(name = "Technology",
                     values = colorsTech,
                     labels = namesTech) +
  labs(x = "Year", y = "Specific capital costs (USD/kWh)") +
  scale_y_continuous(limits = c(0, NA)) +
  ggtitle("Specific capital costs (w/ adjustment costs) of battery storage pack over years") +
  facet_wrap(~ iteration)
...
pe2seTechrm <- gdx::readGDX(rmFiles[1], "pe2se") %>%
...
dataFuelCostsAvgREMIND <- readREMINDgdx(
    ...
    colFilter = list("year" = years, "region" = regions)) %>%
  full_join(pe2seTechrm, by = c("pecarrier"), relationship = "many-to-many") %>%
...
zeroTech <- dataFuelCostsWeightedREMIND %>%
  group_by(technology) %>%
  filter(all(fuelCost == 0 | is.na(fuelCost))) %>%
  pull(technology) %>%
  unique()
...
dataCO2PricePyPSA <- readPyPSAcsv(
    paths = file.path(pyResourcesIterDirs, "scenario_wildcards.csv")) %>% 
...
dataHydroCapPyPSA <- readPyPSAcsv(
    paths = file.path(pyReportingDirs, "optimal_capacities.csv")) %>%
  filter(general_carrier == "PHS & hydro & ror",
         component == "Generator") %>%
...
### Capacity factors (generation technologies)
...
```{r capacity factors generation}

# REMIND capacity factors (anticipation factor based)
dataCapREMIND <- readREMINDgdx(
    ...
    restoreZeros = FALSE) %>%
  filter(!(technology %in% c("elh2", "h2turb", "h2stor", "btin", "btout", "btstor"))) %>%
  select(-grade)

dataUsableSeTeDispREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "v32_pe2seelTe",
    columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "v32_pe2seelTe"),
    colFilter = list("year" = years, "region" = regions, "technology" = names(rm2genTech)),
    restoreZeros = FALSE) %>%
  filter(!(technology %in% c("elh2", "h2turb")))
...
dataCapfacPyPSA <- readPyPSAcsv(
  paths = file.path(pyCouplingDirs, "capacity_factors.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1) %>%
  rename(carrier = general_carrier) %>%
  revalue.levels(carrier = rm2genTech) %>%
  filter(carrier %in% pe2seTech) %>%
...
for (r in regList){
  ...
  dataCapfacREMINDplot <- full_join(dataCapREMIND, dataUsableSeTeDispREMIND) %>%
    ...
    summarise(capfac = sum(v32_pe2seelTe) / sum(vm_cap),
              v32_pe2seelTeRegSum = sum(v32_pe2seelTe)) %>%
    revalue.levels(technology = rm2genTech) %>%
    group_by(year, technology, iteration) %>%
    # Second calculate weighted capacity factor for general technologies
    summarise(capfac = sum(capfac * v32_pe2seelTeRegSum) / sum(v32_pe2seelTeRegSum)) %>%
...
dataMarkupWeightedREMIND <- dataMarkupREMIND %>%
  ...
  revalue.levels(technology = rm2genTech) %>%
  group_by(year, region, technology, iteration) %>%
  summarise(value = sum(value * v32_pe2seelTe) / sum(v32_pe2seelTe)) %>%
  order.levels(technology = names(namesTech))

# PyPSA-Eur
dataMarkupPyPSA <- readPyPSAcsv(
  paths = file.path(pyCouplingDirs, "markups_supply.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1) %>%
  rename(carrier = general_carrier) %>%
  revalue.levels(carrier = rm2genTech) %>%
...
dataMarkupAvgREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "p32_PyPSA_MarkupSupplyAvg",
    columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "p32_PyPSA_MarkupSupplyAvg"),
    colFilter = list("year" = years, "region" = regions, "technology" = names(rm2genTech)),
    restoreZeros = FALSE) %>%
  filter(!is.na(p32_PyPSA_MarkupSupplyAvg)) %>%
  # Increment iteration by 1 because this value is only available for the next iteration
  mutate(iteration = iteration + 1) %>%
  revalue.levels(technology = rm2genTech) %>%
  group_by(year, region, technology, iteration) %>%
  summarise(p32_PyPSA_MarkupSupplyAvg = unique(p32_PyPSA_MarkupSupplyAvg))
...
for (r in regions) {
  ...
  pMarkupYear <- ggplot() +
    ...
    geom_point(data = dataMarkupAvgPlotREMIND, aes(x = year, y = p32_PyPSA_MarkupSupplyAvg, color = technology), alpha = 0.5) +
    scale_shape_manual(name = NULL,
                       values = c("Non-avg. value" = 4)) +
    scale_color_manual(
      name = "Technology",
  ...
  pMarkUpIter <- ggplot() +
    ...
    geom_point(data = dataMarkupAvgREMIND, aes(x = iteration, y = p32_PyPSA_MarkupSupplyAvg, color = technology), alpha = 0.5) +
    scale_shape_manual(name = NULL,
                       values = c("Non-avg. value" = 4)) +
    scale_color_manual(
      name = "Technology",
...
### Markup tax revenue (generation technologies)
...
for (r in regList) {
  ...
  pMarkUpRevenueIter <- ggplot() +
      geom_line(data = dataMarkUpRevenuePlot, aes(x = iteration, y = value)) +
      labs(x = "Iteration", y = "Markup revenue (Million $/yr)") +
      scale_x_continuous(breaks = integer_breaks()) +
      ggtitle("Markup revenue over iterations (supply side)", regSubtitle(r)) +
      facet_wrap(~ year)

  print(pMarkUpRevenueIter)
}
```
...
# REMIND
dataMarkupDemandREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "vm_PyPSAMarkupDemand",
    columns = c("ttot" = "year", "all_regi" = "region", "loadPy32" = "sector", "value" = "value"),
    colFilter = list("year" = years, "region" = regions),
    restoreZeros = FALSE,
    recalcUnit = trUSDpTWa2USDpMWh) %>%
...
# PyPSA electricity price for electrolysis
dataMarkupDemandPyPSA <- readPyPSAcsv(
  paths = file.path(pyCouplingDirs, "markups_demand.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1) %>%
  revalue.levels(general_carrier = py2genTech)

# Plot
pMarkupDemandYear <- ggplot() +
  geom_line(data = dataMarkupDemandREMIND, aes(x = year, y = value, color = sector)) +
  geom_point(data = dataMarkupDemandPyPSA, aes(x = year, y = value, color = general_carrier)) +
  scale_color_manual(name = "Sector",
                     values = colorsTech,
                     labels = namesTech) +
  labs(x = "Year", y = "Markup ($/MWh)") +
  ggtitle("Markup for sectoral electricity prices over years (demand side)") +
  facet_wrap(~ iteration, scales = "free")

print(pMarkupDemandYear)

pMarkupDemandIter <- ggplot() +
  geom_line(data = dataMarkupDemandREMIND, aes(x = iteration, y = value, color = sector)) +
  geom_point(data = dataMarkupDemandPyPSA, aes(x = iteration, y = value, color = general_carrier)) +
  scale_color_manual(name = "Sector",
                     values = colorsTech,
                     labels = namesTech) +
  labs(x = "Iteration", y = "Markup ($/MWh)") +
  scale_x_continuous(breaks = integer_breaks()) +
  ggtitle("Markup for sectoral electricity prices over iterations (demand side)") +
  facet_wrap(~ year, scales = "free")

print(pMarkupDemandIter)

```

### Markup tax revenue (demand side)

```{r markup tax revenue demand}

# REMIND
dataMarkUpRevenue <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "v21_taxrevPyPSAMarkupDemand",
    columns = c("ttot" = "year", "all_regi" = "region", "loadPyMV32" = "sector", "value" = "value"),
    colFilter = list("year" = years, "region" = regions),
    restoreZeros = FALSE,
    recalcUnit = -1E6) %>%  # T$/yr to Million $/yr
  filter(!is.na(sector)) %>%
...
pMarkUpRevenue <- ggplot() +
    geom_line(data = dataMarkUpRevenue, aes(x = year, y = value, color = sector)) +
    scale_color_manual(name = "Sector",
                       values = colorsTech,
                       labels = namesTech) +
    labs(x = "Year", y = "Markup revenue (Million $/yr)") +
    ggtitle("Markup revenue over years (demand side)") +
    facet_wrap(~ iteration)

print(pMarkUpRevenue)

pMarkUpRevenueIter <- ggplot() +
    geom_line(data = dataMarkUpRevenue, aes(x = iteration, y = value, color = sector)) +
    scale_color_manual(name = "Sector",
                       values = colorsTech,
                       labels = namesTech) +
    labs(x = "Iteration", y = "Markup revenue (Million $/yr)") +
    scale_x_continuous(breaks = integer_breaks()) +
    ggtitle("Markup revenue over iterations (demand side)") +
    facet_wrap(~ year)
...
### Peak residual load relative to load
...
dataLoadREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "v32_load",
    columns = c("ttot" = "year", "all_regi" = "region", "value" = "v32_load"),
    colFilter = list("year" = years, "region" = regions),
    restoreZeros = FALSE)

dataPeakResLoadREMIND <- dataDispCapSumREMIND %>%
  left_join(dataLoadREMIND, by = c("year", "region", "iteration")) %>%
  mutate(peakResLoad = dispCapSum / v32_load)

# PyPSA csv contains both absolute and relative
dataPeakResLoadPyPSA <- readPyPSAcsv(
  paths = file.path(pyCouplingDirs, "peak_residual_loads.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1)
...
### Hydrogen storage: Capacity factors

```{r hydrogen storage capacity factors}

# REMIND capacity factors (hydrogen storage)
dataCapFacStorREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "vm_capFac",
    columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "technology" = c("elh2", "h2turb")),
    restoreZeros = FALSE) %>%
  revalue.levels(technology = rm2genTech)

# PyPSA capacita 
dataCapFacStorPyPSA <- readPyPSAcsv(
    paths = file.path(pyCouplingDirs, "capacity_factors.csv")) %>%
  rename(carrier = general_carrier) %>%
  filter(carrier %in% c("elh2", "h2turb")) %>%
  mutate(iteration = iteration + 1) %>%
  revalue.levels(carrier = rm2genTech)

# Plot by year
pCapFacStorYear <- ggplot() +
  geom_line(data = dataCapFacStorREMIND, aes(x = year, y = value, color = technology)) +
  geom_point(data = dataCapFacStorPyPSA, aes(x = year, y = value, color = carrier)) +
  scale_color_manual(name = "Technology",
                     values = colorsTech,
                     labels = namesTech) +
  labs(x = "Year", y = "Capacity factor") +
  ggtitle("Hydrogen storage capacity factors over years") +
  facet_wrap(~ iteration)

print(pCapFacStorYear)

# Plot by iteration
pCapFacStorIter <- ggplot() +
  geom_line(data = dataCapFacStorREMIND, aes(x = iteration, y = value, color = technology)) +
  geom_point(data = dataCapFacStorPyPSA, aes(x = iteration, y = value, color = carrier)) +
  scale_color_manual(name = "Technology",
                     values = colorsTech,
                     labels = namesTech) +
  labs(x = "Iteration", y = "Capacity factor") +
  ggtitle("Hydrogen storage capacity factors over iterations") +
  facet_wrap(~ year)

print(pCapFacStorIter)

```

### Hydrogen storage: Hydrogen turbine share of load

```{r hydrogen storage turbine relative}

# REMIND h2turb production
dataH2turbREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "vm_prodSe",
    columns = c("tall" = "year", "all_regi" = "region", "all_enty" = "enty1", "all_enty1" = "enty2", "all_te" = "technology", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "enty1" = "seh2", "enty2" ="seel", "technology" = "h2turb"),
    restoreZeros = FALSE,
...
# Calculate share of hydrogen turbine w.r.t. load
dataH2turbShareREMIND <- dataH2turbREMIND %>%
  left_join(dataLoadREMIND, by = c("year", "region", "iteration")) %>%
  mutate(value = value / v32_load)

# PyPSA data
dataH2turbPyPSA <- readPyPSAcsv(
    paths = file.path(pyCouplingDirs, "hydrogen_storage_generation.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1)

# Plot
pH2turbRel <- ggplot() +
  geom_line(data = dataH2turbShareREMIND, aes(x = year, y = value, color = region)) +
  geom_point(data = dataH2turbPyPSA, aes(x = year, y = relative, color = region)) +
  labs(x = "Year", y = "Relative hydrogen turbine production") +
  ggtitle("Relative hydrogen turbine production over years") +
  facet_wrap(~ iteration)

print(pH2turbRel)

pH2turbRelIter <- ggplot() +
  geom_line(data = dataH2turbShareREMIND, aes(x = iteration, y = value, color = region)) +
  geom_point(data = dataH2turbPyPSA, aes(x = iteration, y = relative, color = region)) +
  labs(x = "Iteration", y = "Relative hydrogen turbine production") +
  ggtitle("Relative hydrogen turbine production over iterations") +
  facet_wrap(~ year)

print(pH2turbRelIter)

```

### Hydrogen storage: Hydrogen underground storage

```{r hydrogen underground storage}

dataCapH2REMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "vm_cap",
    columns = c("tall" = "year", "all_regi" = "region", "all_te" = "technology", "rlf" = "grade", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "grade" = 1, "technology" = "h2stor"),
    restoreZeros = FALSE)

dataCapH2PyPSA <- readPyPSAcsv(
    paths = file.path(pyCouplingDirs, "optimal_capacities.csv")) %>%
  filter(carrier == "h2stor") %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1) %>%
  mutate(value = value / 1E6)  # MWh to TWh

pCapH2Year <- ggplot() +
  geom_line(data = dataCapH2REMIND, aes(x = year, y = value, color = region)) +
  geom_point(data = dataCapH2PyPSA, aes(x = year, y = value, color = region)) +
  labs(x = "Year", y = "Hydrogen storage capacity (TWh)") +
  ggtitle("Hydrogen storage capacity over years") +
  facet_wrap(~ iteration)

print(pCapH2Year)

pCapH2Iter <- ggplot() +
  geom_line(data = dataCapH2REMIND, aes(x = iteration, y = value, color = region)) +
  geom_point(data = dataCapH2PyPSA, aes(x = iteration, y = value, color = region)) +
  labs(x = "Iteration", y = "Hydrogen storage capacity (TWh)") +
  ggtitle("Hydrogen storage capacity over iterations") +
  facet_wrap(~ year)
...
dataCapFacStorREMIND <- readREMINDgdx(
    ...
    gdxVar = "vm_capFac",
    columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "technology" = c("btin", "btout")),
    restoreZeros = FALSE) %>%
  revalue.levels(technology = rm2genTech)
...
dataCapFacStorPyPSA <- readPyPSAcsv(
    paths = file.path(pyCouplingDirs, "capacity_factors.csv")) %>%
  rename(carrier = general_carrier) %>%
  filter(carrier %in% c("btin", "btout")) %>%
  mutate(iteration = iteration + 1) %>%
  revalue.levels(carrier = rm2genTech)
...
pCapFacStorYear <- ggplot() +
  ...
  scale_color_manual(name = "Technology",
                     ...
                     labels = namesTech) +
  labs(x = "Year", y = "Capacity factor") +
  ggtitle("Battery storage capacity factors over years") +
  facet_wrap(~ iteration)
...
pCapFacStorIter <- ggplot() +
  ...
  scale_color_manual(name = "Technology",
                     ...
                     labels = namesTech) +
  labs(x = "Iteration", y = "Capacity factor") +
  ggtitle("Battery storage capacity factors over iterations") +
  facet_wrap(~ year)
...
### Battery storage: Battery discharging share of load

```{r battery storage relative}

# REMIND battery discharging
dataBatDisREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "vm_prodSe",
    columns = c("tall" = "year", "all_regi" = "region", "all_enty" = "enty1", "all_enty1" = "enty2", "all_te" = "technology", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "enty1" = "seelstor", "enty2" ="seel", "technology" = "btout"),
    restoreZeros = FALSE,
    recalcUnit = twa2twh) %>%  # TWa to TWh
  revalue.levels(technology = rm2genTech) %>%
  select(year, region, technology, value, iteration)
...
# Calculate share of battery discharging w.r.t. load
dataBatDisShareREMIND <- dataBatDisREMIND %>%
  left_join(dataLoadREMIND, by = c("year", "region", "iteration")) %>%
  mutate(value = value / v32_load)

# PyPSA data
dataBatPyPSA <- readPyPSAcsv(
    paths = file.path(pyCouplingDirs, "battery_storage_generation.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1)

# Plot
pBatRelYear <- ggplot() +
  geom_line(data = dataBatDisShareREMIND, aes(x = year, y = value, color = region)) +
  geom_point(data = dataBatPyPSA, aes(x = year, y = relative, color = region)) +
  labs(x = "Year", y = "Relative battery discharging") +
  ggtitle("Battery discharging relative to load over years") +
  facet_wrap(~ iteration)

print(pBatRelYear)

pBatRelIter <- ggplot() +
  geom_line(data = dataBatDisShareREMIND, aes(x = iteration, y = value, color = region)) +
  geom_point(data = dataBatPyPSA, aes(x = iteration, y = relative, color = region)) +
  labs(x = "Iteration",  y = "Relative battery discharging") +
  ggtitle("Battery discharging relative to load over iterations") +
  facet_wrap(~ year)
...
### Battery strorage: Storage pack capacity

```{r battery storage pack}

dataCapBatREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "vm_cap",
    columns = c("tall" = "year", "all_regi" = "region", "all_te" = "technology", "rlf" = "grade", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "grade" = 1, "technology" = "btstor"),
    restoreZeros = FALSE,
    recalcUnit = 1E3)

dataCapBatPyPSA <- readPyPSAcsv(
    paths = file.path(pyCouplingDirs, "optimal_capacities.csv")) %>%
  filter(carrier == "btstor") %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1) %>%
  mutate(value = value / 1E3)  # MWh to GWh

pCapBatYear <- ggplot() +
  geom_line(data = dataCapBatREMIND, aes(x = year, y = value, color = region)) +
  geom_point(data = dataCapBatPyPSA, aes(x = year, y = value, color = region)) +
  labs(x = "Year", y = "Battery storage capacity (GWh)") +
  ggtitle("Battery storage pack capacity over years") +
  facet_wrap(~ iteration)

print(pCapBatYear)

pCapBatIter <- ggplot() +
  geom_line(data = dataCapBatREMIND, aes(x = iteration, y = value, color = region)) +
  geom_point(data = dataCapBatPyPSA, aes(x = iteration, y = value, color = region)) +
  labs(x = "Iteration", y = "Battery storage capacity (GWh)") +
  ggtitle("Battery storage pack capacity over iterations") +
  facet_wrap(~ year)
...
```{r transmission grid losses}

# REMIND grid losses
dataGridLossREMIND <- readREMINDgdx(
    paths = rmFiles,
...
# Calculate share of hydrogen turbine w.r.t. load
dataGridLossRelREMIND <- dataGridLossREMIND %>%
  left_join(dataLoadREMIND, by = c("year", "region", "iteration")) %>%
  mutate(value = value / v32_load)

# PyPSA data
dataGridLossPyPSA <- readPyPSAcsv(
    paths = file.path(pyCouplingDirs, "grid_losses.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1)

# Plot
pGridLossRel <- ggplot() +
  geom_line(data = dataGridLossRelREMIND, aes(x = year, y = value, color = region)) +
  geom_point(data = dataGridLossPyPSA, aes(x = year, y = relative, color = region)) +
  labs(x = "Year", y = "Relative grid losses") +
  ggtitle("Relative grid losses over years") +
  facet_wrap(~ iteration)

print(pGridLossRel)

pGridLossRelIter <- ggplot() +
  geom_line(data = dataGridLossRelREMIND, aes(x = iteration, y = value, color = region)) +
  geom_point(data = dataGridLossPyPSA, aes(x = iteration, y = relative, color = region)) +
  labs(x = "Iteration", y = "Relative grid losses") +
  ggtitle("Relative grid losses over iterations") +
  facet_wrap(~ year)

print(pGridLossRelIter)
...
if (isTRUE(cfg$gms$c32_pypsa_potentials == "on")) {
  ...
  dataPotentialsPyPSA <- readPyPSAcsv(
      paths = file.path(pyCouplingDirs, "potentials.csv")) %>%
  ...
  dataPotentialsREMIND <- readREMINDgdx(
      ...
      recalcUnit = 1e3) %>%  # TW to GW
    revalue.levels(technology = rm2genTech) %>%
    filter(technology %in% unique(dataPotentialsPyPSA$carrier),
           !is.infinite(value)) %>%
    group_by(year, region, technology) %>%
    summarise(value = unique(value))
...
### Anticipation: Generation shares

```{r anticipation generation shares}

dataGenShareREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "v32_shPe2seel",
    columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "technology" = names(rm2genTech)),
    restoreZeros = FALSE)

dataGenSharePyPSA <- readPyPSAcsv(
  paths = file.path(pyCouplingDirs, "generation_shares.csv")) %>%
...
### Anticipation: Difference quotient (capacity factors)

```{r anticipation difference quotient}

if (cfg$gms$c32_pypsa_anticipation == "diffQuot") {

    dataDQREMIND <- readREMINDgdx(
        paths = rmFiles,
        gdxVar = "p32_PyPSA_DQ_CF",
        columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "all_te1" = "tech_perturbed", "value" = "value"),
        colFilter = list("year" = years, "region" = regions),
        restoreZeros = FALSE) %>%
      mutate(value = replace_na(value, 0)) %>%
      revalue.levels(technology = rm2genTech,
                     tech_perturbed = rm2genTech) %>%
      group_by(year, region, technology, tech_perturbed, iteration) %>%
      summarise(value = unique(value))

    dataDQPyPSA <- readPyPSAcsv(
        paths = file.path(pyCouplingDirs, "difference_quotient_capacity_factors.csv")) %>%
      rename(technology = general_carrier,
             tech_perturbed = carrier_perturbed) %>%
      revalue.levels(technology = rm2genTech,
                     tech_perturbed = rm2genTech) %>%
      group_by(year, region, technology, tech_perturbed, iteration) %>%
      summarise(value = unique(value))
    ...
    pDQYear <- ggplot() +
      geom_line(data = dataDQREMIND, aes(x = year, y = value, color = tech_perturbed)) +
      geom_point(data = dataDQPyPSA, aes(x = year, y = value, color = tech_perturbed)) +
      scale_color_manual(name = "Perturbed technology",
                         values = colorsTech,
                         labels = namesTech) +
      labs(x = "Year", y = "Difference quotient of CF w.r.t. capacity") +
      ggtitle("Difference quotients for capacity factors over years") +
      facet_grid(technology ~ iteration, scales = "free", labeller = labeller(technology = namesTech))

    print(pDQYear)

    pDQIter <- ggplot() +
      geom_line(data = dataDQREMIND, aes(x = iteration, y = value, color = tech_perturbed)) +
      geom_point(data = dataDQPyPSA, aes(x = iteration, y = value, color = tech_perturbed)) +
      scale_color_manual(name = "Perturbed technology",
                         values = colorsTech,
                         labels = namesTech) +
      labs(x = "Iteration", y = "Difference quotient") +
      ggtitle("Difference quotients for capacity factors over iterations") +
      facet_grid(technology ~ year, scales = "free", labeller = labeller(technology = namesTech))
    
    print(pDQIter)

    for (i in pyIterations) {
      dataDQREMINDi <- dataDQREMIND %>%
        filter(iteration == i)
      
      dataDQPyPSAi <- dataDQPyPSA %>%
        filter(iteration == i)

      pDQIter <- ggplot() +
        geom_line(data = dataDQREMINDi, aes(x = year, y = value, color = tech_perturbed)) +
        geom_point(data = dataDQPyPSAi, aes(x = year, y = value, color = tech_perturbed)) +
        scale_color_manual(name = "Perturbed technology",
                           values = colorsTech,
                           labels = namesTech) +
        labs(x = "Year", y = "Difference quotient") +
        ggtitle("Difference quotients for capacity factors over years", paste0("Iteration ", i))

      pDQiterScalesFixed <- pDQIter +
        facet_grid(tech_perturbed ~ technology,
                   labeller = labeller(technology = namesTech, tech_perturbed = namesTech))

      print(pDQiterScalesFixed)

      pDQiterScalesFree <- pDQIter +
        facet_grid(tech_perturbed ~ technology,
                   labeller = labeller(technology = namesTech, tech_perturbed = namesTech),
                   scales = "free")

      print(pDQiterScalesFree)
    }
}

```

### Anticipation: Difference quotient (markups)

### Anticipation: Optimal capacities of REMIND technologies

```{r anticipation optimal capacities}

tePy32 <- gdx::readGDX(rmFiles[1], "tePy32")

dataOptCapREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "vm_cap",
    columns = c("tall" = "year", "all_regi" = "region", "all_te" = "technology", "rlf" = "grade", "value" = "vm_cap"),
    colFilter = list("year" = years, "region" = regions, "grade" = 1, "technology" = names(rm2genTech)),
    restoreZeros = FALSE,
    recalcUnit = 1e3) %>%
  filter(technology %in% tePy32)

dataOptCapPyPSA <- readPyPSAcsv(
    paths = file.path(pyCouplingDirs, "optimal_capacities.csv")) %>%
  mutate(value = value / 1E3) %>%  # MW to GW
  filter(carrier %in% tePy32) %>%
  rename(technology = carrier)

unique_technologies <- unique(dataOptCapREMIND$technology)
...
for (t in techsets) {
  ...
  dataOptCapREMINDplot <- dataOptCapREMIND %>%
    filter(technology %in% t)

  dataOptCapPyPSAplot <- dataOptCapPyPSA %>%
    filter(technology %in% t)


  pYear <- ggplot() +
    geom_line(data = dataOptCapREMINDplot, aes(x = year, y = vm_cap, color = "REMIND")) +
    geom_line(data = dataOptCapPyPSAplot, aes(x = year, y = value, color = "PyPSA")) +
    labs(x = "Year", y = "Capacity (GW)") +
    ggtitle("Optimal capacities over years") +
    facet_grid(technology ~ iteration, scales = "free", labeller = labeller(technology = namesTech))

  print(pYear)

  pIter <- ggplot() +
    geom_line(data = dataOptCapREMINDplot, aes(x = iteration, y = vm_cap, color = "REMIND")) +
    geom_line(data = dataOptCapPyPSAplot, aes(x = iteration, y = value, color = "PyPSA")) +
    labs(x = "Iteration", y = "Capacity (GW)") +
    scale_x_continuous(breaks = integer_breaks()) +
    ggtitle("Optimal capacities over iterations") +
    facet_grid(technology ~ year, scales = "free", labeller = labeller(technology = namesTech))
...
### Relative channge over iterations (tech dimension)

```{r conv metric}

categoryNames <- c(
  "CF" = "Capacity factor",
  "MarkupSupply" = "Supply-side markups",
  "MarkupDemand" = "Demand-side markups",
  "PeakResLoadRel" = "Peak residual load",
  "OptCap" = "Optimal storage capacity",
  "Potential" = "Potential",
  "H2TurbRel" = "Hydrogen turbine share",
  "BatteryDischargeRel" = "Battery discharge share",
  "GridLossesRel" = "Grid losses share"
)

lastIter <- max(pyIterations)
last_fulldata_file <- rmFiles[str_detect(rmFiles, paste0("fulldata_", lastIter, "\\.gdx$"))]

dataDeltaTech <- readGDX(last_fulldata_file, "p32_delta_tech", restore_zeros = FALSE) %>%
  as_tibble() %>%
  filter(all_regi %in% regions,
         all_te %in% names(rm2genTech),
         paramsPyTech32 != "shPe2seel") %>%
  rename(region = all_regi, technology = all_te, category = paramsPyTech32) %>%
  revalue.levels(technology = rm2genTech) %>%
  group_by(region, technology, category, iteration) %>%
  summarise(test = unique(value))

p <- ggplot() +
  geom_line(data = dataDeltaTech, aes(x = iteration, y = test, color = technology)) +
  scale_color_manual(name = "Technology", values = colorsTech, labels = namesTech) +
  labs(x = "Iteration", y = "Relative change") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  ggtitle("Relative change of variables with tech dimension over iterations (agg. over time)") +
  facet_grid(category ~ ., scales = "free", labeller = labeller(category = categoryNames))

print(p)

```

### Relative change over iterations (load dimension)

```{r conv metric load}

dataDeltaLoad <- readGDX(last_fulldata_file, "p32_delta_load", restore_zeros = FALSE) %>%
  as_tibble() %>%
  filter(all_regi %in% regions) %>%
  rename(region = all_regi, load = loadPy32, category = paramsPyLoad32) %>%
  revalue.levels(load = rm2genTech)

pLoad <- ggplot() +
  geom_line(data = dataDeltaLoad, aes(x = iteration, y = value, color = load)) +
  scale_color_manual(name = "Load", values = colorsTech, labels = namesTech) +
  labs(x = "Iteration", y = "Relative change") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  ggtitle("Relative change of variables with load dimension over iterations (agg. over time)") +
  facet_wrap(category ~., scales = "free", labeller = labeller(category = categoryNames))

print(pLoad)  

```

### Relative change over iterations (scalars)

```{r conv metric scalars}

dataScalarLoad <- readGDX(last_fulldata_file, "p32_delta_scalar", restore_zeros = FALSE) %>%
  as_tibble() %>%
  filter(all_regi %in% regions) %>%
  rename(region = all_regi, category = paramsPyScalar32)

p <- ggplot() +
  geom_line(data = dataScalarLoad, aes(x = iteration, y = value)) +
  labs(x = "Iteration", y = "Relative change") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  ggtitle("Relative change of scalar variables over iterations (agg. over time)") +
  facet_wrap(category ~ ., scales = "free", labeller = labeller(category = categoryNames))
...
dataDeltaTechAgg <- readGDX(last_fulldata_file, "p32_delta_weighted", restore_zeros = FALSE) %>%
  as_tibble() %>%
  filter(all_regi %in% regions,
         paramsPy32 != "shPe2seel") %>%
  rename(region = all_regi, category = paramsPy32)

pAgg <- ggplot() +
  geom_line(data = dataDeltaTechAgg, aes(x = iteration, y = value)) +
  labs(x = "Iteration", y = "Relative change") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  ggtitle("Relative change of categories over iterations (agg. over time and techs)") +
  facet_wrap(~ category, scales = "free", labeller = labeller(category = categoryNames))

print(pAgg)

threshold <- readGDX(last_fulldata_file, "p32_conv_threshold", restore_zeros = FALSE) %>%
  as_tibble() %>%
  filter(all_regi %in% regions) %>%
  rename(region = all_regi, category = paramsPy32)
...
pConv <- pAgg +
  ...
  geom_hline(data = threshold, aes(yintercept = value, color = "Threshold"), linetype = "dashed") +
  scale_color_manual(name = NULL, values = c("Threshold" = "darkred"))
...
### Optimal generation capacities line charts
...
dataCapREMIND <- readREMINDgdx(
    ...
    recalcUnit = 1e3) %>%   # TW to GW
  select(-grade) %>%
  filter(!(technology %in% c("h2stor", "btstor")))

# PyPSA capacities
dataCapPyPSA <- readPyPSAcsv(
    paths = file.path(pyReportingDirs, "optimal_capacities.csv")) %>%
  filter(component %in% c("Generator", "Link"),
         !(general_carrier %in% c("heat pump", "resistive heating", "BEV charger", "DC"))) %>%
  revalue.levels(general_carrier = py2genTech) %>%
  rename(technology = general_carrier) %>%
  mutate(value = value / 1E3)  # MW to GW

# PyPSA capacity is defined w.r.t. input, not output
# E.g., for electrolysis the capacity is defined w.r.t. electrical input
# Multiply with efficiency to get output capacity like in REMIND
dataEtaPyPSA <- readPyPSAcsv(
  paths = file.path(pyResourcesDirs, "costs.csv")) %>%
...
dataCapPyPSA <- dataCapPyPSA %>%
  left_join(dataEtaPyPSA, by = c("year", "technology", "iteration")) %>%
  mutate(value = ifelse(component == "Link", value * eta, value))
...
### Optimal storage capacities line charts

```{r optimal storage capacities line charts}
...
dataCapREMIND <- readREMINDgdx(
    ...
    gdxVar = "vm_cap",
    columns = c("tall" = "year", "all_regi" = "region", "all_te" = "technology", "rlf" = "grade", "value" = "vm_cap"),
    colFilter = list("year" = years, "region" = regions, "technology" = c("h2stor", "btstor"), "grade" = 1),
    restoreZeros = FALSE,
    recalcUnit = 1E3) %>%  # TWh to GWh
  select(-grade) %>%
  revalue.levels(technology = rm2genTech)
...
dataCapPyPSA <- readPyPSAcsv(
    paths = file.path(pyReportingDirs, "optimal_capacities.csv")) %>%
  filter(general_carrier %in% c("H2", "battery")) %>%
  rename(carrier = general_carrier) %>%
...
for (r in regList) {
  ...
  pYear <- ggplot() +
    ...
    scale_color_manual(name = "Technology", values = colorsTech, labels = namesTech) +
    scale_linetype_manual(name = NULL, values = c("REMIND" = "solid", "PyPSA-Eur" = "dashed")) +
    labs(x = "Year", y = "Capacity (GWh)") +
    ggtitle("Optimal storage capacities over years", regSubtitle(r))
  ...
  pIter <- ggplot() +
    ...
    scale_color_manual(name = "Technology", values = colorsTech, labels = namesTech) +
    scale_linetype_manual(name = NULL, values = c("REMIND" = "solid", "PyPSA-Eur" = "dashed")) +
    labs(x = "Iteration", y = "Capacity (GWh)") +
    scale_x_continuous(breaks = integer_breaks()) +
    ggtitle("Optimal storage capacities over iterations", regSubtitle(r))
...
dataPreInvCapAvgREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "p32_capAvg",
    columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "p32_capAvg"),
    colFilter = list("year" = years, "region" = regions, "technology" = names(rm2genTech)),
    restoreZeros = TRUE,
    recalcUnit = 1e3) %>%
  revalue.levels(technology = rm2genTech) %>%
  group_by(year, region, technology, iteration) %>%
  summarise(p32_capAvg = sum(p32_capAvg))
...
dataCapInvestREMIND <- full_join(dataPreInvCapREMIND, dataCapREMIND) %>%
  mutate(vm_cap = vm_cap,
         deltaCap = vm_cap - p32_cap) %>%
  revalue.levels(technology = rm2genTech) %>%
  group_by(year, region, technology, iteration) %>%
  summarise(deltaCap = sum(deltaCap),
            preInvCap = sum(p32_cap)) %>%
  order.levels(technology = names(colorsTech)) %>%
...
dataPreInvCapPyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "preinstalled_capacities.csv")) %>%
  rename(carrier = general_carrier) %>%
  # Convert MW to GW and MWh to GWh
  mutate(value = value / 1e3) %>%
  rename(preInvCap = value) %>%
  # Separate capacities from ppl (powerplantmatching database) and RCL constraint
  pivot_wider(names_from = RCL, values_from = preInvCap) %>%
  rename(capPPL = False,
         capRCL = True) %>%
  mutate(capPPL = ifelse(is.na(capPPL), 0, capPPL),
         capRCL = ifelse(is.na(capRCL), 0, capRCL))

dataCapPyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "optimal_capacities.csv")) %>%
  rename(carrier = general_carrier) %>%
  filter(carrier %in% (dataPreInvCapPyPSA %>% pull(carrier) %>% unique())) %>%
  # Convert MW to GW and MWh to GWh
  mutate(value = value / 1e3) %>%
  select(-component) %>%
  rename(cap = value)

# Get efficiencies for links
dataEtaPyPSA <- readPyPSAcsv(
  paths = file.path(pyResourcesDirs, "costs.csv")) %>%
  filter(parameter == "efficiency") %>%
  revalue.levels(technology = py2genTech) %>%
...
dataCapInvestPyPSA <- full_join(dataPreInvCapPyPSA, dataCapPyPSA) %>%
  ...
  # Adjust by efficiencies for links
  left_join(dataEtaPyPSA) %>%
  # Insert placeholder to also show battery charger/discharger
  mutate(component = ifelse(is.na(component), "Other", component)) %>%
  mutate(cap = ifelse(component == "Link", cap * eta, cap),
         capPPL = ifelse(component == "Link", capPPL * eta, capPPL),
         capRCL = ifelse(component == "Link", capRCL * eta, capRCL)) %>%
  mutate(deltaCap = cap - capPPL - capRCL) %>%
  rename(technology = carrier) %>%
  order.levels(technology = names(colorsTech)) %>%
  pivot_longer(cols = c("deltaCap", "capPPL", "capRCL"),
               names_to = "variable",
               values_to = "value") %>%
  order.levels(variable = c("deltaCap", "capRCL", "capPPL"))
  ...
  for (i in pyIterations){
    ...
    pCapInvest <- ggplot() +
      ...
      geom_point(data = dataPreInvCapAvgREMINDplot,
                mapping = aes(x = year, y = p32_capAvg, fill = technology, shape = "Averaged"),
                color = "black",
      ...
      scale_alpha_manual(
        name = "Capacity",
        values = c("deltaCap" = 0.4,
                   "preInvCap" = 1,
                   "capRCL" = 0.7,
                   "capPPL" = 1),
        labels = c("deltaCap" = "Investment",
                   "preInvCap" = "Pre-investment",
                   "capRCL" = "Pre-inv. (greenfield)",
                   "capPPL" = "Pre-inv. (brownfield)")) +
      scale_fill_manual(
        name = "Technology",
        values = colorsTech,
        labels = namesTech) +
      ...
      guides(linetype = guide_legend(override.aes = list(fill = NA)),
             fill = "none") +
      labs(x = "Year", y = "Capacity (GW or GWh)") +
...
dataPeakResLoadMinREMIND <- full_join(dataPeakResLoadPyPSA %>% mutate(iteration = iteration + everyIter),
                                      dataGenREMIND) %>%
  mutate(minCap = relative * v32_load)

dataPeakResLoadAbsPyPSA <- readPyPSAcsv(
    paths = file.path(pyReportingDirs, "peak_residual_load_absolute.csv")) %>%
...
dataEtaPyPSA <- readPyPSAcsv(
  paths = file.path(pyResourcesDirs, "costs.csv")) %>%
  filter(parameter == "efficiency",
         technology %in% c("battery inverter", "fuel cell")) %>%
  revalue.levels(technology = c("battery inverter" = "battery_discharger",
                                "fuel cell" = "fuel_cell")) %>%
  rename(eta = value) %>%
  mutate(eta = ifelse(technology == "battery_discharger", eta**0.5, eta)) %>%
  select(year, technology, iteration, eta)

# Adjust link capacities for eta
dataCapPyPSAplot <- dataCapPyPSA %>%
  revalue.levels(technology = py2genTech) %>%
  filter(technology %in% supplyTech) %>%
  group_by(year, region, component, technology, iteration) %>%
  summarise(value = sum(value)) %>%
  full_join(dataEtaPyPSA) %>%
  mutate(value = ifelse(component == "Link", value * eta, value)) %>%
  order.levels(technology = supplyTech)
...
for (r in regList){
  for (i in pyIterations){
    ...
    dataPeakResLoadPyPSAplot <- dataPeakResLoadAbsPyPSA %>%
      filter(region %in% r,
             iteration == i) %>%
      group_by(year) %>%
...
### Electricity balance charts side-by-side

```{r electricity balance charts side-by-side}
...
dataBalancePyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "energy_balance_ac.csv")) %>%
  filter(component != "StorageUnit") %>%
  mutate(value = value / 1e6) %>%  # MWh to TWh
  rename(technology = general_carrier) %>%
  mutate(technology = ifelse(component == "Load", "load", technology),
         technology = ifelse(component == "Line", "line_losses", technology)) %>%
  revalue.levels(technology = py2genTech) %>%
  mutate(technology = factor(technology, levels = names(colorsTech)))

# Positive values are supply
dataSupplyPyPSA <- dataBalancePyPSA %>%
  filter(value > 0)

# Negative values are withdrawal
dataWithdrawalPyPSA <- dataBalancePyPSA %>%
  filter(value < 0) %>%
  order.levels(technology = rev(names(colorsTech)))

# Get additional hydrogen load from PyPSA-Eur
dataH2DemandPyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "energy_balance_addH2.csv")) %>%
  filter(component == "Load") %>%
  mutate(value = -value / 1e6) %>%
  complete(year = years, iteration = pyIterations, fill = list(value = 0, region = "DEU"))

# Divide by efficiency to get electricity demand for electrolysis
# for additional hydrogen demand
dataEtaPyPSA <- readPyPSAcsv(
  paths = file.path(pyResourcesDirs, "costs.csv")) %>%
  filter(parameter == "efficiency",
...
dataElectrolysisAddPyPSA <- dataH2DemandPyPSA %>%
  left_join(dataEtaPyPSA) %>%
  mutate(value = value / eta) %>%
  mutate(technology = "electrolysis_add") %>%
  select(year, region, technology, iteration, value)

dataElectrolysisStorPyPSA <- dataWithdrawalPyPSA %>%
  filter(technology == "electrolysis") %>%
  mutate(value = -value) %>%
  full_join(dataElectrolysisAddPyPSA, by = c("year", "region", "iteration")) %>%
  mutate(value = value.x - value.y) %>%
  mutate(technology = "electrolysis_stor") %>%
  select(year, region, technology, iteration, value)

dataElectrolysisPyPSA <- bind_rows(
    dataElectrolysisAddPyPSA,
    dataElectrolysisStorPyPSA) %>%
  mutate(value = -value)

dataWithdrawalPyPSA <- dataWithdrawalPyPSA %>%
  filter(technology != "electrolysis") %>%
  select(year, region, technology, iteration, value) %>%
  bind_rows(dataElectrolysisPyPSA)
...
if (isTRUE(cfg$gms$c32_pypsa_trade == "off")) {
  for (r in regList) {
    for (i in rmIterations){
      ...
      dataSupplyREMINDploti <- dataSupplyREMIND %>%
        filter(region %in% r,
              iteration == i) %>%
        group_by(year, technology) %>%
        summarise(value = sum(value))

      dataSupplyREMINDplotitot <- dataSupplyREMINDploti %>%
        group_by(year) %>%
        summarise(value = sum(value))

      dataWithdrawalREMINDploti <- dataWithdrawalREMIND %>%
        filter(region %in% r,
               iteration == i) %>%
        group_by(year, technology) %>%
        summarise(value = sum(value)) %>% 
      ...
      dataWithdrawalREMINDplotitot <- dataWithdrawalREMINDploti %>%
        group_by(year) %>%
        summarise(value = sum(value))

      dataSupplyPyPSAploti <- dataSupplyPyPSA %>%
        filter(region %in% r,
              iteration == i) %>%
        group_by(year, technology) %>%
        summarise(value = sum(value))

      dataSupplyPyPSAplotitot <- dataSupplyPyPSAploti %>%
        group_by(year) %>%
        summarise(value = sum(value))

      dataWithdrawalPyPSAploti <- dataWithdrawalPyPSA %>%
        filter(region %in% r,
              iteration == i) %>%
        group_by(year, technology) %>%
        summarise(value = sum(value)) %>% 
      ...
      dataWithdrawalPyPSAplotitot <- dataWithdrawalPyPSAploti %>%
        group_by(year) %>%
        summarise(value = sum(value))

      pGenComp <- ggplot() +
        # REMIND supply
        geom_col(data = dataSupplyREMINDploti,
                mapping = aes(x = year, y = value, fill = technology),
                position = position_stacknudge(x = -1),
                width = 1.7) +
        # PyPSA-Eur supply
        geom_col(data = dataSupplyPyPSAploti,
                mapping = aes(x = year, y = value, fill = technology),
                position = position_stacknudge(x = 1),
                width = 1.7) +
        # Supply legend
        ...
        new_scale_fill() +
        # REMIND withdrawal
        geom_col(data = dataWithdrawalREMINDploti,
                mapping = aes(x = year, y = value, fill = technology),
                position = position_stacknudge(x = -1),
                width = 1.7) +
        # PyPSA-Eur withdrawal
        geom_col(data = dataWithdrawalPyPSAploti,
                mapping = aes(x = year, y = value, fill = technology),
                position = position_stacknudge(x = 1),
                width = 1.7) +
        # Withdrawal legend
        ...
        # Model border
        geom_col(data = dataSupplyREMINDplotitot,
                mapping = aes(x = year, y = value, linetype = "REMIND"),
                position = position_nudge(x = -1),
                width = 1.7,
                linewidth = 0.25,
                color = "black",
                fill = NA) +
        geom_col(data = dataWithdrawalREMINDplotitot,
                mapping = aes(x = year, y = value, linetype = "REMIND"),
                position = position_nudge(x = -1),
                width = 1.7,
                linewidth = 0.25,
                color = "black",
                fill = NA) +
        geom_col(data = dataSupplyPyPSAplotitot,
                mapping = aes(x = year, y = value, linetype = "PyPSA-Eur"),
                position = position_nudge(x = 1),
                width = 1.7,
                linewidth = 0.25,
                color = "black",
                fill = NA) +
        geom_col(data = dataWithdrawalPyPSAplotitot,
                mapping = aes(x = year, y = value, linetype = "PyPSA-Eur"),
                position = position_nudge(x = 1),
                width = 1.7,
                linewidth = 0.25,
        ...
        scale_x_continuous(breaks = years) +
        labs(x = "Year", y = "Energy (TWh)") +
        ggtitle(paste("Electricity balance over years, iteration", i), regSubtitle(r))

      # Plot relative differences
      dataGenDiffplot <- full_join(dataSupplyREMINDploti, dataSupplyPyPSAploti,
                        by = c("year", "technology")) %>%
      ...
      pGenDiff <- ggplot() + 
        ...
        labs(x = "Year", y = "Relative difference (%)") +
        ggtitle("Relative difference w.r.t total supply in REMIND")
...
dataElecPricePyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "load_prices.csv")) %>% 
  rename(carrier = general_carrier) %>%
  filter(carrier == "total") %>%
  select(!carrier)
...
pElecPriceYear <- ggplot() +
  ...
  scale_y_continuous(limits = c(0, NA)) +
  ggtitle("Average electricity price over years")
...
pElecPriceIter <- ggplot() +
  ...
  scale_x_continuous(breaks = rmIterations) +
  scale_y_continuous(limits = c(0, NA)) +
  ggtitle("Average electricity price over iterations")
...
# REMIND hydrogen price
dataH2PriceREMIND <- readREMINDgdx(
  paths = rmFiles,
  gdxVar = "pm_SEPrice",
  columns = c("ttot" = "year", "all_regi" = "region", "all_enty" = "secarrier", "value" = "value"),
  colFilter = list("year" = years, "region" = regions, "secarrier" = "seh2"),
  restoreZeros = FALSE,
  recalcUnit = trUSDpTWa2USDpMWh) %>%  # T$/TWa to $/MWh
  select(-secarrier)

# PyPSA hydrogen price
dataH2PricePyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "load_prices.csv")) %>%
  rename(carrier = general_carrier) %>%
  filter(carrier == "H2 demand REMIND") %>% 
  select(!carrier)

pH2PriceYear <- ggplot() +
  geom_line(data = dataH2PriceREMIND,
            mapping = aes(x = year, y = value, color = "REMIND")) +
  geom_line(data = dataH2PricePyPSA,
            mapping = aes(x = year, y = value, color = "PyPSA-Eur")) +
  scale_color_discrete(name = "Model") +
  labs(x = "Year", y = "Hydrogen price ($/MWh)") +
  scale_y_continuous(limits = c(0, NA)) +
  ggtitle("Hydrogen price over years") +
  facet_wrap(~ iteration, scales = "free")
...
pH2PriceIter <- ggplot() +
  geom_line(data = dataH2PriceREMIND,
            mapping = aes(x = iteration, y = value, color = "REMIND")) +
  geom_line(data = dataH2PricePyPSA,
            mapping = aes(x = iteration, y = value, color = "PyPSA-Eur")) +
  scale_color_discrete(name = "Model") +
  labs(x = "Iteration", y = "Hydrogen price ($/MWh)") +
  scale_x_continuous(breaks = rmIterations) +
  scale_y_continuous(limits = c(0, NA)) +
  ggtitle("Hydrogen price over iterations") +
  facet_wrap(~ year, scales = "free")
...
lcoeREMIND <- bind_rows(lcoe.investment, lcoe.fom, lcoe.vom, lcoe.fuel, lcoe.co2) %>%
  ...
  summarise(lcoe = sum(lcoe * v32_pe2seelTe) / sum(v32_pe2seelTe))
...
# Read data
dataElecBalance <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "p32_ElecBalance",
    columns = c("ttot" = "year", "all_regi" = "region", "rep32" = "component", "value" = "value"),
    colFilter = list("year" = years, "region" = regions),
    restoreZeros = FALSE,
    recalcUnit = twa2twh) %>%
...
# Component mapping
mapElecBalanceSupply <- c(
  "1" = "PE generation",
  "2" = "SE generation (hydrogen turbine)",
  "3" = "Coproduction SE",
  "4" = "Coproduction FE",
  "5" = "Coproduction CCS",
  "6" = "Import")

mapElecBalanceWithdrawal <- c(
  "7" = "FE demand",
  "8" = "SE demand (electrolysis)",
  "9" = "Storage losses (v32_storloss)",
  "10" = "Own consumption fuel extraction",
  "11" = "Export",
  "12" = "Grid losses")

# Supply
dataElecBalanceSupply <- dataElecBalance %>%
  filter(component %in% names(mapElecBalanceSupply)) %>%
  mutate(component = mapElecBalanceSupply[component])

# Withdrawal
dataElecBalanceWithdrawal <- dataElecBalance %>%
  filter(component %in% names(mapElecBalanceWithdrawal)) %>%
  mutate(component = mapElecBalanceWithdrawal[component],
          value = -value)

# Net
dataElecBalanceNet <- bind_rows(dataElecBalanceSupply, dataElecBalanceWithdrawal) %>%
  mutate(value = replace_na(value, 0)) %>%
  group_by(year, region, iteration) %>%
  summarise(value = sum(value))
...
# Plot over years
pElecBalance <- ggplot() +
  geom_col(data = dataElecBalanceSupply,
          aes(x = year, y = value, fill = component),
          position = "stack") +
  scale_fill_discrete(name = "Supply (positive)",
                      guide = guide_legend(order = 1)) +
  new_scale_fill() +
  geom_col(data = dataElecBalanceWithdrawal,
          aes(x = year, y = value, fill = component),
          position = "stack") +
  scale_fill_discrete(name = "Withdrawal (negative)",
                      guide = guide_legend(order = 2)) +
  geom_line(data = dataElecBalanceNet,
            aes(x = year, y = value, color = "Net")) +
  scale_color_manual(
    name = "Net",
    values = "black",
    guide = guide_legend(order = 3)) +
  labs(x = "Year", y = "Electricity balance (TWh)") +
  ggtitle("Electricity balance equation") +
  facet_wrap(~iteration)

print(pElecBalance)

# Plot over iterations
pElecBalanceIter <- ggplot() +
  geom_col(data = dataElecBalanceSupply,
          aes(x = iteration, y = value, fill = component),
          position = "stack") +
  scale_fill_discrete(name = "Supply (positive)",
                      guide = guide_legend(order = 1)) +
  new_scale_fill() +
  ...
  scale_fill_discrete(name = "Withdrawal (negative)",
                      guide = guide_legend(order = 2)) +
  geom_line(data = dataElecBalanceNet,
            aes(x = iteration, y = value, color = "Net")) +
  scale_color_manual(
    name = "Net",
    values = "black",
    guide = guide_legend(order = 3)) +
  labs(x = "Iteration", y = "Electricity balance (TWh)") +
  ggtitle("Electricity balance equation") +
  facet_wrap(~year)

print(pElecBalanceIter)  
...
dataMarketValuePyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "market_values_supply.csv")) %>%
  rename(carrier = general_carrier) %>%
  revalue.levels(carrier = py2genTech) %>%
  group_by(year, region, carrier, iteration) %>%
  summarise(value = unique(value)) %>%
...
pMarketValueYear <- ggplot() +
  geom_line(data = dataMarketValuePyPSA, aes(x = year, y = value, color = carrier, linetype = "No")) +
  geom_line(data = dataMarketValuePyPSAcutoff, aes(x = year, y = value, color = carrier, linetype = "Yes")) +
  scale_linetype_manual(
      name = "Peak prices",
      values = c("No" = "solid", "Yes" = "dashed"),
      labels = c("No" = "No cutoff", "Yes" = "With cutoff"),
      guide = guide_legend(order = 1)
  ) +
  scale_color_manual(
    name = "Technology",
...
pMarketValueYearCutoff <- ggplot() +
  geom_line(data = dataMarketValuePyPSAcutoff, aes(x = year, y = value, color = carrier)) +
  scale_color_manual(
    name = "Technology",
    values = colorsTech,
    labels = namesTech,
  ...
  labs(x = "Year", y = "Market value ($/MWh)") +
  ggtitle("Market values by year (peak hours cut off)")

if (length(regions) > 1) {
  pMarketValueYearCutoff <- pMarketValueYearCutoff + facet_grid(region ~ iteration, scales = "free")
} else {
  pMarketValueYearCutoff <- pMarketValueYearCutoff + facet_wrap(~ iteration)
}

print(pMarketValueYearCutoff)

pMarketValueIter <- ggplot() +
  geom_line(data = dataMarketValuePyPSA, aes(x = iteration, y = value, color = carrier, linetype = "No")) +
  geom_line(data = dataMarketValuePyPSAcutoff, aes(x = iteration, y = value, color = carrier, linetype = "Yes")) +
  scale_linetype_manual(
      name = "Peak prices",
      values = c("No" = "solid", "Yes" = "dashed"),
      labels = c("No" = "No cutoff", "Yes" = "With cutoff"),
...
pMarketValueIterCutoff <- ggplot() +
  geom_line(data = dataMarketValuePyPSAcutoff, aes(x = iteration, y = value, color = carrier)) +
  scale_color_manual(
    name = "Technology",
    values = colorsTech,
    labels = namesTech,
  ...
  labs(x = "Iteration", y = "Market value ($/MWh)") +
  ggtitle("Market values by iteration (peak hours cut off)")

if (length(regions) > 1) {
  pMarketValueIterCutoff <- pMarketValueIterCutoff + facet_grid(region ~ year, scales = "free")
} else {
  pMarketValueIterCutoff <- pMarketValueIterCutoff + facet_wrap(~ year)
}

print(pMarketValueIterCutoff)

```

### Sectoral electricity prices (demand side)

```{r end-use electricity prices}

dataElectricityPrices <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "load_prices.csv")) %>%
  rename(carrier = general_carrier) %>%
  revalue.levels(carrier = py2genTech)

dataElectricityPricesCutoff <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "load_prices_cutoff.csv")) %>%
  rename(carrier = general_carrier) %>%
  revalue.levels(carrier = py2genTech)

dataElectricityPriceAvg <- dataElectricityPrices %>%
    filter(carrier == "total")

dataElectricityPricesAvgCutoff <- dataElectricityPricesCutoff %>%
    filter(carrier == "total")

p <- ggplot() +
  geom_line(data = dataElectricityPrices, aes(x = year, y = value, color = carrier, linetype = "No")) +
  geom_line(data = dataElectricityPricesCutoff, aes(x = year, y = value, color = carrier, linetype = "Yes")) +
  scale_linetype_manual(
    name = "Peak prices",
    values = c("No" = "solid", "Yes" = "dashed"),
    labels = c("No" = "No cutoff", "Yes" = "With cutoff"),
  ...
  scale_color_manual(
    ...
    values = colorsTech) +
  new_scale_color() +
  geom_line(data = dataElectricityPriceAvg,
            aes(x = year, y = value, color = "Average"),
            linewidth = 1) +
  geom_line(data = dataElectricityPricesAvgCutoff,
            aes(x = year, y = value, color = "Average"),
            linewidth = 1, linetype = "dashed") +
  scale_color_manual(name = NULL, values = c("Average" = "black")) +
  labs(x = "Year", y = "Electricity price ($/MWh)") +
  ggtitle("End-use electricity prices") +
  facet_wrap(~ iteration)

print(p)

pIter <- ggplot() +
  geom_line(data = dataElectricityPrices, aes(x = iteration, y = value, color = carrier, linetype = "No")) +
  geom_line(data = dataElectricityPricesCutoff, aes(x = iteration, y = value, color = carrier, linetype = "Yes")) +
  scale_linetype_manual(
    name = "Peak prices",
    values = c("No" = "solid", "Yes" = "dashed"),
    labels = c("No" = "No cutoff", "Yes" = "With cutoff"),
  ...
  new_scale_color() +
  geom_line(data = dataElectricityPriceAvg,
            aes(x = iteration, y = value, color = "Average"),
            linewidth = 1) +
  geom_line(data = dataElectricityPricesAvgCutoff,
            aes(x = iteration, y = value, color = "Average"),
            linewidth = 1, linetype = "dashed") +
...
dataCurtailmentPyPSA <- readPyPSAcsv(
    paths = file.path(pyReportingDirs, "curtailments.csv")) %>%
  filter(value != 0) %>%
  rename(carrier = general_carrier) %>%
  # Increment iteration by 1 to match REMIND
  revalue.levels(carrier = py2genTech) %>%
  ...
  mutate(value = value / 1e6) %>%  # Convert MWh to TWh
  rename(technology = carrier) %>%
  # Only include for VRE technologies
  filter(technology %in% c("solar_pv", "wind_offshore", "wind_onshore", "hydro"))
...
for (r in regList) {
  ...
  pCurtailmentYear <- ggplot() +
    geom_line(data = dataCurtailmentplotPyPSA, aes(x = year, y = value, color = technology)) +
    scale_color_manual(
      name = "Technology",
      values = colorsTech,
      labels = namesTech,
  ...
  pCurtailmentIter <- ggplot() +
    geom_line(data = dataCurtailmentplotPyPSA, aes(x = iteration, y = value, color = technology)) +
    scale_color_manual(
      name = "Technology",
      values = colorsTech,
      labels = namesTech,
    ) +
    labs(x = "Iteration", y = "Curtailment (TWh/yr)") +
    scale_x_continuous(breaks = rmIterations) +
...
```{r price duration curves}

dataHourlyElecPricesPyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "hourly_prices.csv")) %>%
  pivot_longer(cols = -c("year", "iteration"),
               names_to = "time",
               values_to = "value") %>%
  group_by(year, iteration) %>%
  mutate(hour = 1:n()) %>%
...
pPriceDuration <- ggplot() +
  geom_line(data = dataHourlyElecPricesPyPSA, aes(x = year.fraction, y = value, alpha = as.factor(year))) +
  scale_alpha_discrete(name = "Year") +
  labs(x = "Hours of year (fraction)", y = "Electricity price ($/MWh)") +
  ggtitle("Price duration curve") +
  facet_wrap(~ iteration)

print(pPriceDuration)

pPriceDurationLog <- ggplot() +
  geom_line(data = dataHourlyElecPricesPyPSA, aes(x = year.fraction, y = value, alpha = as.factor(year))) +
  scale_alpha_discrete(name = "Year") +
  labs(x = "Hours of year (fraction)", y = "Electricity price ($/MWh)") +
  ggtitle("Price duration curve, log scale") +
  scale_y_continuous(trans = "log10") +
  facet_wrap(~ iteration)

print(pPriceDurationLog)

pPriceDurationCut <- ggplot() +
  geom_line(data = dataHourlyElecPricesPyPSA, aes(x = year.fraction, y = value, alpha = as.factor(year))) +
  scale_alpha_discrete(name = "Year") +
  labs(x = "Hours of year (fraction)", y = "Electricity price ($/MWh)") +
  coord_cartesian(ylim = c(0, 200)) +
  ggtitle("Price duration curve, cut off at 200 $/MWh") +
  facet_wrap(~ iteration)

print(pPriceDurationCut)

```

### Price duration curves (last iteration)

```{r price duration curves last iteration}

# Price duration curves for last iteration only
maxiter <- dataHourlyElecPricesPyPSA %>%
  ungroup() %>%
  pull(iteration) %>%
  max()

dataHourlyElecPricesPyPSA <- dataHourlyElecPricesPyPSA %>%
  ungroup() %>%
  filter(iteration == maxiter)

pPriceDuration <- ggplot() +
  geom_line(data = dataHourlyElecPricesPyPSA, aes(x = year.fraction, y = value)) +
  labs(x = "Hours of year (fraction)", y = "Electricity price ($/MWh)") +
  ggtitle(paste("Price duration curve, iteration", maxiter)) +
  facet_wrap(~ year)
...
pPriceDurationCut <- ggplot() +
  geom_line(data = dataHourlyElecPricesPyPSA, aes(x = year.fraction, y = value)) +
  labs(x = "Hours of year (fraction)", y = "Electricity price ($/MWh)") +
  ggtitle(paste("Price duration curve, cut off at 200 $/MWh, iteration", maxiter)) +
  coord_cartesian(ylim = c(0, 200)) +
  facet_wrap(~ year)
...
```{r screening curves, eval=FALSE}

# Create vector with strings from costs_2025.csv, costs_2030.csv, to costs_2150.csv
costsFiles <- as.vector(outer(pyReportingDirs, paste0("costs_", years, ".csv"), paste, sep = "/"))

# Read PyPSA-Eur costs
dataCostsPyPSA <- readPyPSAcsv(
  paths = costsFiles) %>%
  select(-X) %>%
  filter(parameter %in% c("capital_cost", "marginal_cost"),
         technology %in% names(py2genTech)) %>%
  revalue.levels(technology = py2genTech) %>%
  # Only include dispatchable technologies
  filter(technology %in% c("CCGT", "OCGT", "biomass", "all_coal", "nuclear", "oil")) %>%
  group_by(technology, parameter, iteration, year) %>%
  summarise(value = unique(value))

# For each technology, iteration and year, create a 
dataCostsPyPSAplot <- dataCostsPyPSA %>%
  group_by(technology, iteration, year) %>%
  mutate(x.val = ifelse(parameter == "capital_cost", 0, 8760),
         y.val = ifelse(parameter == "capital_cost", value, lag(value) + value * 8760))

# Read PyPSA-Eur capacity factors
dataCapfacPyPSA <- readPyPSAcsv(
  paths = file.path(pyValidationDirs, "capacity_factors.csv")) %>%
  revalue.levels(carrier = rm2genTech) %>%
  rename(technology = carrier) %>%
  group_by(year, region, technology, iteration) %>%
  summarise(value = unique(value)) %>%
  mutate(capfac.h = value * 8760) %>%
  select(-value)

# Merge with dataCostsPyPSAplot
dataCostsPyPSAplot <- dataCostsPyPSAplot %>%
  full_join(dataCapfacPyPSA, by = c("technology", "iteration", "year")) %>%
  mutate(x.val.cf = ifelse(parameter == "capital_cost", capfac.h, 0),
         y.val.cf = ifelse(parameter == "capital_cost", value + capfac.h * lead(value), lag(y.val)))

for (i in pyIterations) {

  dataCostsPyPSAploti <- dataCostsPyPSAplot %>%
    filter(iteration == i)

  p <- ggplot() +
    geom_line(data = dataCostsPyPSAploti,
              mapping = aes(x = x.val, y = y.val, color = technology),
              linetype = "dotted") +
    geom_line(data = dataCostsPyPSAploti,
              mapping = aes(x = x.val.cf, y = y.val.cf, color = technology),
              linetype = "solid") +
    scale_color_manual(
      name = "Technology",
      values = colorsTech,
    ...
    labs(x = "Hours", y = "Total cost ($/MW)") +
    ggtitle(paste("Screening curves for dispatchable technologies in PyPSA-Eur, iteration", i)) +
    facet_wrap(~ year)
...
### PyPSA network (last iteration)

```{python network plot, echo=FALSE, warning = FALSE, message = FALSE}
...
# Suppress INFO and WARNING statements
logging.getLogger('pypsa').setLevel(logging.WARNING)

# Get PyPSA-Eur results directories from R
# FOR TESTING: Set py_results_dirs manually
py_results_dirs = r.pyResultsDirs

# Get all network files
fp = [file for directory in py_results_dirs for file in Path(directory).rglob("y*/networks/*.nc")]

# Remove all files with a _op in their name
fp = [file for file in fp if "_op" not in str(file)]

# Extract the year and iteration from the file path and store it in a DataFrame
df = pd.DataFrame(fp, columns=['file_path'])

# Convert PosixPath objects to strings and extract the year and iteration
df['file_path_str'] = df['file_path'].astype(str)
df['year'] = df['file_path_str'].str.extract(r'y(\d{4})')
df['iteration'] = df['file_path_str'].str.extract(r'i(\d+)')

# Arrange by year and iteration
df['year'] = pd.to_numeric(df['year'])
df['iteration'] = pd.to_numeric(df['iteration'])
df = df.sort_values(by=['year', 'iteration'])

# Only show the last iteration
...
fig, axs = plt.subplots(3, 5, subplot_kw={"projection": ccrs.EqualEarth()})

# Counter to keep track of subplot index
counter = 0

# Filter for the current iteration and then iterate over the years
for _, row in df.iterrows():
    
    # Load the PyPSA network
    n = pypsa.Network(row['file_path'])
    ...
    buses = n.generators.groupby(["bus", "carrier"]).p_nom_opt.sum()
    ...
    n.plot(ax=ax,
        ...
        title=f"Year: {row['year']}");
    
    # Make the title smaller
    ax.title.set_fontsize(8)
    
    ax.set_extent([6, 15, 47, 55], crs=ccrs.PlateCarree())  # Adjust the values as needed
    
    counter += 1
    
# Add one big heading to the entire plot for the iteration
plt.tight_layout(rect=[0, 0, 1, 0.95])  # Adjust the rect parameter to make space for the suptitle
# Add title
fig.suptitle(f"PyPSA network: capacities, links and lines", fontsize=10, y=0.98);
plt.show();
plt.close(fig);

```

### Electricity balance, summer week

```{python electricity balance summer, echo=FALSE, warning = FALSE, message = FALSE}
...
# Get the first config file (to get colours)
fp_config = next(Path(py_results_dirs[0]).rglob("y*/configs/*.yaml"), None)

# Read first config yaml file
with open(fp_config, 'r') as file:
    config = yaml.safe_load(file)

# Convert the tech_colors list to a pandas Series
color_series = pd.Series(config['plotting']['tech_colors'])

# Add colors for resistive heating
color_series['resistive'] = "#B86B1F"  # Red for resistive heating
color_series['Load'] = "#4B5C67"
color_series['heatpump'] = "#F5835A"
color_series["BEV pass charger"] = "#E8E88A"
color_series["BEV freight charger"] = "#68B774"

# Variables to store handles and labels for the legend
...
for _, row in df.iterrows():
    ...
    # If network has no objective attribute skip the iteration and year
    if not hasattr(n, 'objective'):
        counter += 1
        continue

    # Use statistics.energy_balance
    p = (
    ...
    # Rename first column to Load
    p.rename(columns={"": "Load"}, inplace=True)
    
    # Filter for a week in July
    ...
    # Create a plot for the electricity balance
    ax = axs[counter // 5, counter % 5]

    # Plot positive values (supply)
    p_positive.plot.area(ax=ax, stacked=True, linewidth=0, color=[color_series[col] for col in p_positive.columns], legend=False);

    # Plot negative values (withdrawal)
    p_negative.plot.area(ax=ax, stacked=True, linewidth=0, color=[color_series[col] for col in p_negative.columns], legend=False);

    # Update global y-axis limits
    global_min = min(global_min, p_negative.sum(axis=1).min())
    global_max = max(global_max, p_positive.sum(axis=1).max())
...
fig.legend(handles, labels, loc='center left', bbox_to_anchor=(0.87, 0.5), ncol=1);

plt.tight_layout(rect=[0, 0, 0.85, 0.95]);  # Adjust the rect parameter to make space for the legend
fig.suptitle(f"Generation mix in summer", fontsize=10, y=0.98);
plt.show();
plt.close(fig);

```

### Electricity balance, winter week

```{python electricity balance winter, echo=FALSE, warning = FALSE, message = FALSE}
...
plt.tight_layout(rect=[0, 0, 0.85, 0.95]);  # Adjust the rect parameter to make space for the legend
fig.suptitle(f"Generation mix in winter", fontsize=10, y=0.98);
plt.show();
plt.close(fig);

```

### State of charge (hydrogen)

```{python state of charge}

def plot_storage_soc(df, color_line, carrier, time_slice=None, bev_e_min=False, hp_e_max=False):
    """
    Plot state of charge for a given storage carrier across multiple years.

    Parameters:
        df (pd.DataFrame): DataFrame with 'year' and 'file_path' columns.
        color_line (str): Color for the line representing the state of charge.
        carrier (str): Name of the carrier (e.g. 'battery', 'hydrogen').
        time_slice (slice or None): Optional datetime slice for zooming in.
    """
    n_rows, n_cols = 3, 5
    ...
    for _, row in df.iterrows():
        ...
        # Skip if no objective (incomplete run)
        if not hasattr(n, 'objective'):
            continue

        # Extract state of charge for the specified carrier
        soc = n.stores_t.e.loc[:, n.stores.carrier == carrier].sum(axis=1)

        # If EV battery, also plot e_min_pu * e_nom
        if bev_e_min:
            e_min_pu = n.stores_t.e_min_pu.loc[:, n.stores.carrier == carrier].mean(axis=1)
            e_nom = n.stores.e_nom[n.stores.carrier == carrier].sum()
            e_min = e_min_pu * e_nom

        # If heat pump storage, also plot e_max_pu * e_nom
        if hp_e_max:
            e_max_pu = n.stores_t.e_max_pu.loc[:, n.stores.carrier == carrier].mean(axis=1)
            e_nom = n.stores.e_nom[n.stores.carrier == "heatpump storage"].sum()
            e_max = e_max_pu * e_nom

        # Optionally apply a time slice (e.g. zoom to a specific week)
        if time_slice:
            soc = soc.loc[time_slice]
            if bev_e_min:
                e_min = e_min.loc[time_slice] if bev_e_min else None
            if hp_e_max:
                e_max = e_max.loc[time_slice] if hp_e_max else None

        # Plot on the current subplot
        ax = axs[counter]
        soc.plot(ax=ax, color=color_line)

        if bev_e_min:
            e_min.plot(ax=ax, label="Minimum SOC", color = "red", linestyle='--')

        if hp_e_max:
            e_max.plot(ax=ax, label="Maximum SOC", color = "red", linestyle='--')

        ax.set_title(f"Year: {row['year']}")
        ax.set_xlabel('')
        ax.set_ylabel('SOC [MWh]' if counter % n_cols == 0 else '')
    ...
    for i in range(counter, n_rows * n_cols):
        fig.delaxes(axs[i])

    # Final layout and display
    fig.suptitle(f"State of Charge for {carrier}", fontsize=12, y=0.98)
    plt.tight_layout(rect=[0, 0, 0.95, 0.95]);
    plt.show();
    plt.close(fig);

plot_storage_soc(df, color_series["H2"], 'H2')
...
plot_storage_soc(df, color_series["battery"], 'battery')
plot_storage_soc(df, color_series["battery"], 'battery', time_slice=slice('2013-07-15', '2013-07-21'))

```

### State of charge (EV passenger batteries)

```{python soc ev pass}

plot_storage_soc(df, color_series["BEV pass charger"], 'EV pass battery', bev_e_min=False)
plot_storage_soc(df, color_series["BEV pass charger"], 'EV pass battery', time_slice=slice('2013-07-15', '2013-07-21'), bev_e_min=True)

```

### State of charge (EV freight batteries)

```{python soc ev freight}

plot_storage_soc(df, color_series["BEV freight charger"], 'EV freight battery', bev_e_min=False)
plot_storage_soc(df, color_series["BEV freight charger"], 'EV freight battery', time_slice=slice('2013-07-15', '2013-07-21'), bev_e_min=True)

```


### State of charge (heat pump storage)

```{python soc hp}

plot_storage_soc(df, color_series['heatpump'], 'heatpump storage', hp_e_max=True)
plot_storage_soc(df, color_series['heatpump'], 'heatpump storage', time_slice=slice('2013-01-21', '2013-01-27'), hp_e_max=True)

```

### State of charge (resistive heating storage)

```{python soc resistive}

plot_storage_soc(df, color_series['resistive'], 'resistive storage')
plot_storage_soc(df, color_series['resistive'], 'resistive storage', time_slice=slice('2013-01-21', '2013-01-27'))

```

### System costs

```{python system costs, echo=FALSE, warning = FALSE, message = FALSE}
...
# Initialize variables to track global max for y-axis
...
for _, row in df.iterrows():
    ...
    capex_cost = n.statistics.capex(nice_names=False).to_frame()
    installed_capex_cost = n.statistics.installed_capex(nice_names=False).to_frame()
    opex_cost = n.statistics.opex(nice_names=False).to_frame()

    # Exclude H2 transfer to H2 demand REMIND links (can have costs due to noisy_costs)
    opex_cost = opex_cost.drop(index=opex_cost.filter(like="H2 demand", axis="rows").index)

    # Calculate system costs
    system_cost = capex_cost.subtract(installed_capex_cost, fill_value=0).add(opex_cost, fill_value=0)
    system_cost = system_cost.reset_index()[["carrier", 0]].rename(columns={0: "cost"}).set_index("carrier")

    system_cost = system_cost.T
    
    # Objective value (total system costs) for cross checking

    # Try to get objective, if not available leave empty
    try:
    ...
    # Show system costs
    ax = axs[counter // 5, counter % 5]
    system_cost.plot(ax=ax,
        kind="bar",
        stacked=True,
        color = [color_series[col] for col in system_cost.columns],
        legend=False,
        title=f"Year: {row['year']}");
    
    # Add a cross for the value of n.objective
    if objective is not None:
        ax.scatter(x=0, y=objective, color='black', marker='x', s=10, label='Objective');
    ...
    # Update global max for y-axis
    global_max = max(global_max, system_cost.values.sum())
...
fig.legend(handles, labels, loc='center left', bbox_to_anchor=(0.87, 0.5), ncol=1);
plt.tight_layout(rect=[0, 0, 0.85, 0.95]);  # Adjust the rect parameter to make space for the legend
fig.suptitle(f"System costs", fontsize=10, y=0.99)
plt.show();
plt.close(fig);
...
dataCapexREMIND <- readREMINDgdx(
    ...
    colFilter = list("year" = years, "region" = regions, "technology" = names(rm2genTech)),
    restoreZeros = FALSE,
    recalcUnit = 1E-3*trUSDpTW2USDpMW) %>%
  filter(iteration %in% pyIterations)
...
cost_years <- c(2025, 2030, 2035, 2040, 2045, 2050)

dataCapexPyPSA <- tibble()
for (y in cost_years) {
  # Read costs from PyPSA-Eur
  temp <- readPyPSAcsv(
    paths = file.path(pyDir, paste0("resources/costs_", y, ".csv"))) %>%
    filter(parameter == "investment",
           technology %in% names(costMap)) %>%
    mutate(year = y)

  dataCapexPyPSA <- bind_rows(dataCapexPyPSA, temp)
}

dataPlotPyPSA <- dataCapexPyPSA %>%
  revalue.levels(technology = costMap) %>%
  select(technology, year, value)

dataPlotREMIND <- dataCapexWeightedREMIND %>%
  filter(iteration == max(iteration),
         technology %in% costMap) %>%
  select(technology, year, capex) %>%
  rename(value = capex)

pTech <- ggplot() +
  geom_line(data = dataPlotREMIND, aes(x = year, y = value, color = technology, linetype = "REMIND")) +
  geom_line(data = dataPlotPyPSA, aes(x = year, y = value, color = technology, linetype = "PyPSA-Eur")) +
  scale_color_manual(name = "Technology",
                     values = colorsTech,
                     labels = namesTech) +
...
pTech <- pTech +
    coord_cartesian(xlim = c(2025, 2050))
...
# For electrolysis, convert USD/kW to USD/kW_el
# Don't bother for batteries as efficiency is almost 100%
dataEtaConvREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "pm_eta_conv",
    columns = c("tall" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "eta"),
  ...
  filter(iteration == max(iteration),
         technology == "elh2") %>%
  mutate(technology = "electrolysis")

dataPlotREMIND <- dataPlotREMIND %>%
  full_join(dataEtaConvREMIND) %>%
  mutate(value = ifelse(technology == "electrolysis", value / eta, value))
...
p <- ggplot() +
  ...
  scale_linetype_manual(name = "Model",
                     values = c("REMIND" = "solid", "PyPSA-Eur" = "dashed")) +
  labs(x = "Year", y = "Specific capital costs (USD/kW and USD/kWh)") +
  ylim(0, NA) +
  ggtitle("Cost comparison") +
  facet_wrap(~ technology, scales = "free", labeller = labeller(technology = namesTech))
...
```{r mv demand presentation, eval = FALSE}

dataMVdemand <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "market_values_demand.csv")) %>%
  rename(carrier = general_carrier) %>%
  filter(iteration == 20)

dataMVdemandcutoff <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "market_values_demand_cutoff.csv")) %>%
  rename(carrier = general_carrier) %>%
  filter(iteration == 20)

dataLoadPrice <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "load_prices.csv")) %>%
  rename(carrier = general_carrier) %>%
  filter(carrier == "AC") %>%
  mutate(carrier = "load") %>%
...
dataLoadPricecutoff <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "load_prices_cutoff.csv")) %>%
  rename(carrier = general_carrier) %>%
  filter(carrier == "AC") %>%
  mutate(carrier = "load") %>%
...
pMVdemand <- ggplot() +
  geom_line(data = dataMVdemand, aes(x = year, y = value, color = carrier, linetype = "Not cut off")) +
  geom_line(data = dataMVdemandcutoff, aes(x = year, y = value, color = carrier, linetype = "Cut off")) +
  scale_color_discrete(name = "Demand") +
  new_scale_color() +
  geom_line(data = dataLoadPrice, aes(x = year, y = value, color = "Total load", linetype = "Not cut off")) +
  geom_line(data = dataLoadPricecutoff, aes(x = year, y = value, color = "Total load", linetype = "Cut off")) +
  scale_color_manual(name = NULL,
                     values = c("Total load" = "black")) +
  scale_linetype_manual(name = "Peak prices",
                        values = c("Not cut off" = "solid", "Cut off" = "dashed")) +
  labs(x = "Year", y = "Electricity price ($/MWh)") +
  ggtitle("End-use electricity prices w/ flexible EVs") +
  scale_y_continuous(limits = c(0, 85))

ggsave("mv_demand_ev_dsm.png", width = 16, height = 10, units = "cm", dpi = 300, bg = "white")

...
dataLoadPyPSA <- readPyPSAcsv(
    paths = file.path(pyValidationDirs, "loads.csv")) %>%
  filter(carrier == "AC") %>%
...
for (r in regList){
  ...
  dataCapfacPyPSAplot <- dataCapfacPyPSA %>%
    ...
    if (nrow(dataCapfacREMINDplot) > 0) {
        print("Data frame dataCapfacREMINDplot is not empty.")
    } else {
        print("Data frame dataCapfacREMINDplot is empty.")
    }

    if (nrow(dataCapfacPyPSAplot) > 0) {
        print("Data frame dataCapfacPyPSAplot is not empty.")
    } else {
        print("Data frame dataCapfacPyPSAplot is empty.")
    }

    if (nrow(dataCapfacAvgREMINDplot) > 0) {
        print("Data frame dataCapfacAvgREMINDplot is not empty.")
    } else {
        print("Data frame dataCapfacAvgREMINDplot is empty.")
    }

    # Check if data frames are not empty
    if (nrow(dataCapfacREMINDplot) > 0 && nrow(dataCapfacPyPSAplot) > 0 && nrow(dataCapfacAvgREMINDplot) > 0) {
        ...
        print("One or more data frames are empty. Skipping plots.")
    }
}
```
...
```{r markups}
# REMIND (anticipation factor based)
...
dataMarkupPyPSA <- readPyPSAcsv(
  paths = file.path(pyValidationDirs, "markups.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1) %>%
...
dataMarkupAvgREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "p32_PyPSA_MarkupAvg",
    columns = c("ttot" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "p32_PyPSA_MarkupAvg"),
    colFilter = list("year" = years, "region" = regions, "technology" = names(rm2genTech)),
    restoreZeros = FALSE) %>%
  # Increment iteration by 1 because this value is only available for the next iteration
  mutate(iteration = iteration + 1) %>%
  revalue.levels(technology = rm2genTech) %>%
  group_by(year, region, technology, iteration) %>%
  summarise(p32_PyPSA_MarkupAvg = unique(p32_PyPSA_MarkupAvg))
...
### Markup: Electricity prices for electrolysis
...
dataElecPriceElectrolysisPyPSA <- readPyPSAcsv(
  paths = file.path(pyValidationDirs, "electricity_prices_electrolysis.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1)

# PyPSA load price
dataLoadPricePyPSA <- readPyPSAcsv(
...
# Calculate markup
dataMarkupElectrolysisPyPSA <- dataElecPriceElectrolysisPyPSA %>%
  left_join(dataLoadPricePyPSA, by = c("year", "region", "iteration")) %>%
...
dataCapFacStorREMIND <- readREMINDgdx(
    ...
    restoreZeros = FALSE) %>%
  revalue.levels(technology = c("elh2" = "electrolysis", "h2turb" = "fuel_cell"))

# PyPSA capacita 
dataCapFacStorPyPSA <- readPyPSAcsv(
    paths = file.path(pyValidationDirs, "capacity_factors_links.csv")) %>%
    filter(carrier %in% c("H2 electrolysis", "H2 fuel cell")) %>%
  mutate(iteration = iteration + 1) %>%
  revalue.levels(carrier = c("H2 electrolysis" = "electrolysis", "H2 fuel cell" = "fuel_cell"))
...
dataH2turbPyPSA <- readPyPSAcsv(
    paths = file.path(pyValidationDirs, "h2turb_storages.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1)
...
```{r hydrogen storage turbine absolute}

pH2turbAbs <- ggplot() +
  geom_line(data = dataH2turbREMIND, aes(x = year, y = value, color = region)) +
  geom_point(data = dataH2turbPyPSA, aes(x = year, y = absolute/1E6, color = region)) +
  labs(x = "Year", y = "Hydrogen turbine production (TWh)") +
  ggtitle("Hydrogen turbine production over years", "Not directly coupled to REMIND") +
  facet_wrap(~ iteration)
...
dataCapFacStorPyPSA <- readPyPSAcsv(
    paths = file.path(pyValidationDirs, "capacity_factors_links.csv")) %>%
    filter(carrier %in% c("battery charger", "battery discharger")) %>%
  mutate(iteration = iteration + 1) %>%
...
dataBatPyPSA <- readPyPSAcsv(
    paths = file.path(pyValidationDirs, "battery_storages.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1)
...
dataGridLossPyPSA <- readPyPSAcsv(
    paths = file.path(pyValidationDirs, "grid_losses.csv")) %>%
  # Increment iteration by 1 to match REMIND
  mutate(iteration = iteration + 1)
...
dataCapPyPSA <- dataCapPyPSA %>%
  ...
  mutate(value = ifelse(type == "Link", value * eta, value)) %>%
  filter(technology %in% rm2genTech,
         type != "Store")
...
dataCapPyPSA <- readPyPSAcsv(
    paths = file.path(pyValidationDirs, "optimal_capacities.csv")) %>%
  filter(carrier %in% c("H2", "battery")) %>%
  revalue.levels(carrier = c("H2" = "hydrogen_storage", "battery" = "battery_storage")) %>%
  rename(technology = carrier) %>%
  mutate(value = value / 1E3)  # MWh to GWh
...
dataCapInvestPyPSA <- full_join(dataPreInvCapPyPSA, dataCapPyPSA) %>%
  ...
  mutate(cap = ifelse(component == "Link", cap * eta, cap),
         preInvCap = ifelse(component == "Link", preInvCap * eta, preInvCap)) %>%
...
dataCapPyPSAplot <- dataCapPyPSA %>%
  revalue.levels(technology = py2genTech) %>%
  filter(technology %in% supplyTech) %>%
  group_by(year, region, type, technology, iteration) %>%
  summarise(value = sum(value)) %>%
  full_join(dataEtaPyPSA) %>%
  mutate(value = ifelse(type == "Link", value * eta, value)) %>%
  order.levels(technology = supplyTech)
...
dataGenPyPSA <- readPyPSAcsv(
  paths = file.path(pyValidationDirs, "energy_balances.csv")) %>%
  mutate(value = value / 1e6) %>%  # MWh to TWh
  rename(technology = carrier) %>%
  revalue.levels(technology = py2genTech) %>%
  filter(technology %in% supplyTech) %>%
    ...
    for (i in rmIterations){
      ...
      pGenComp <- ggplot() +
        ...
        scale_fill_manual(
          ...
          breaks = c("load", "line_losses", "electrolysis_stor", "electrolysis_add", "battery_charger"),
...
## Anticipation parametrisation

### Generation shares of REMIND technologies

Generation shares are not explicitly coupled between REMIND and PyPSA-Eur, but used to parametrise the anticipation factor equations to avoid oscillations. If they don't converge, there is a always a distortion in the variables passed from PyPSA-Eur to REMIND.
...
costsFiles <- as.vector(outer(pyValidationDirs, paste0("costs_", years, ".csv"), paste, sep = "/"))
...
```{python residual load duration curve, echo=FALSE, warning = FALSE, message = FALSE}

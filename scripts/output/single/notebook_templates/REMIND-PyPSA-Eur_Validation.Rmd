...
### Electricity load (REMIND2PyPSAEUR_*.gdx)
...
pYear <- ggplot() +
  ...
  geom_point(data = dataLoad, aes(x = year, y = value, color = region)) +
...
pIter <- ggplot() +
  ...
  geom_point(data = dataLoad, aes(x = iteration, y = value, color = region)) +
...
### Electricity load (used by PyPSA-EUR networks)

```{r load, echo = FALSE, message = FALSE}
# Read the electricity loads from the CSV files
dataElecLoadPyPSA <- readPyPSAcsv(
...
dataElecLoadPyPSA$value <- dataElecLoadPyPSA$value / 1e6 # Convert MWh to TWh

# Plot by year
pYear <- ggplot() +
  geom_line(data = dataElecLoadPyPSA, aes(x = year, y = value, color = region)) +
  geom_point(data = dataElecLoadPyPSA, aes(x = year, y = value, color = region)) +
  labs(x = "Year", y = "Electricity load (TWh)") +
  ggtitle("Electricity load by year") +
  facet_wrap(~ iteration)
...
# Plot by iteration
pIter <- ggplot() +
  geom_line(data = dataElecLoadPyPSA, aes(x = iteration, y = value, color = region)) +
  geom_point(data = dataElecLoadPyPSA, aes(x = iteration, y = value, color = region)) +
  labs(x = "Iteration", y = "Electricity load (TWh)") +
  ggtitle("Electricity load by iteration") +
  facet_wrap(~ year)
...
### CO2 prices (used by PyPSA-EUR networks)

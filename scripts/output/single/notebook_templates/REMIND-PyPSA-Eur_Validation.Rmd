...
pyResultsDirs <- paste0(pyResultsDirs, "/")
...
readPyPSAcsv <- function(paths) {
  ...
  for (p in paths) {
    ...
    iteration <- as.integer(str_extract(p, "(?<=i)\\d(?=//)"))
    # Read data
...
dataLoadPyPSA <- readPyPSAcsv(
  ...
  mutate(iteration = as.integer(iteration + 1),)

dataLoadPyPSA$value <- dataLoadPyPSA$value / 1e6 # Convert MWh to TWh
...
dataCO2PricePyPSA$value <- as.numeric(str_extract(dataCO2PricePyPSA$opts, "(?<=Ep)\\d+\\.\\d{1,2}"))

# Plot by year
pYear <- ggplot() +
  geom_line(data = dataCO2PricePyPSA, aes(x = year, y = value)) +
...
# Plot by iteration
pIter <- ggplot() +
  geom_line(data = dataCO2PricePyPSA, aes(x = iteration, y = value)) +
...
# Load optimal_capacities.csv for each iteration and plot the capacities by type and carrier in bar plots
# with subplots for each year and iterations being lines within the same plot

# Read the optimal capacities from the CSV files
...
dataCapPyPSA$value <- dataCapPyPSA$value / 1e3

# Remove rows with nan values and rows with carrier == "Load shedding" which has a fixed capacity
dataCapPyPSA <- dataCapPyPSA %>% 
  filter(!is.nan(value)) %>% 
  filter(value > 0.1) %>% 
  filter(carrier != "Load shedding")

# Sum over all carriers and group the results by type
dataCapPyPSA_type <- dataCapPyPSA %>% 
...
pType <- ggplot() +
  geom_point(data = dataCapPyPSA_type, aes(x = year, y = value, color = iteration)) +
  geom_line(data = dataCapPyPSA_type, aes(x = year, y = value, color = iteration)) +
...
pYear <- ggplot() +
  geom_bar(data = dataCapPyPSA, aes(x = carrier, y = value, fill=factor(year)), stat = "identity", position_dodge(width = 0.9)) +

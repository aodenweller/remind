...
for (r in regList){
  ...
  pCapfacYear <- ggplot() +
    ...
    # Layer 2: PyPSA-Eur
    geom_point(data = dataCapfacPyPSAplot, aes(x = year, y = value, color = technology)) +
    # Legend formatting
    scale_color_manual(
      name = "Technology",
      values = colorsTech,
  ...
  pCapfacIter <- ggplot() +
    ...
    # Layer 2: PyPSA-Eur
    geom_point(data = dataCapfacPyPSAplot, aes(x = iteration, y = value, color = technology)) +
    # Legend formatting
    scale_color_manual(
      name = "Technology",
      values = colorsTech,
...
# Calculate "marginal LCOEs", that is LCOEs for new plants, also see remind2/reportLCOE.R
...
dataElecPlusMarkupREMIND <- dataElecPriceREMIND %>%
  ...
  select(year, region, technology, value, iteration) %>%
  mutate(type = "Electricity price + Markup")

# Peak capacity shadow price
dataPeakCapMargREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "q32_PeakResCap",
    columns = c("ttot" = "year", "all_regi" = "region", "value" = "q32_PeakResCap"),
    colFilter = list("year" = years, "region" = regions),
    restoreZeros = FALSE,
    field = "m")

dataBudgetMargREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "qm_budget",
    columns = c("ttot" = "year", "all_regi" = "region", "value" = "qm_budget"),
    colFilter = list("year" = years, "region" = regions),
    restoreZeros = FALSE,
    field = "m")

dataPeakCapShadowPriceREMIND <- dataPeakCapMargREMIND %>%
  left_join(dataBudgetMargREMIND) %>%
  left_join(capfac) %>%
  filter(!(technology %in% teVRE)) %>%
  mutate(peakShadowPrice = q32_PeakResCap / qm_budget / capfac * trUSDpTWa2USDpMWh) %>%
  select(year, region, iteration, technology, peakShadowPrice)

dataElecPlusMarkupPlusPeakShadowREMIND <- dataElecPlusMarkupREMIND %>%
  left_join(dataPeakCapShadowPriceREMIND) %>%
  mutate(value = value + peakShadowPrice) %>%
  select(year, region, technology, value, iteration) %>%
  mutate(type = "Electricity price + Markup + Peak capacity shadow price")

dataPricesREMIND <- bind_rows(dataElecPriceREMIND, dataElecPlusMarkupREMIND, dataElecPlusMarkupPlusPeakShadowREMIND) %>%
  order.levels(type = c("Electricity price + Markup + Peak capacity shadow price", "Electricity price + Markup", "Electricity price"))
...
p <- ggplot() +
  geom_bar(data = lcoeREMIND %>% filter(iteration == 14),
           aes(x = year, y = lcoe, fill = type),
           stat = "identity",
           position = "stack",
           alpha = 0.8) +
  geom_line(data = dataPricesREMIND %>% filter(iteration == 14),
            aes(x = year, y = value, linetype = type),
            linewidth = 0.5) +
  scale_linetype_manual(
    name = NULL,
    values = c("Electricity price + Markup + Peak capacity shadow price" = "solid",
               "Electricity price + Markup" = "dashed",
               "Electricity price" = "dotted"),
    guide = guide_legend(order = 1)
  ) +
  scale_fill_manual(
    name = "Cost component",

...
rm2genTech <- c(rm2genTech, "pc" = "all_coal")  # Remove later
...
pCO2PriceYear <- ggplot() +
  ...
  geom_point(data = dataCO2PricePyPSA, aes(x = year, y = value, color = region)) +
...
pCO2PriceIter <- ggplot() +
  ...
  geom_point(data = dataCO2PricePyPSA, aes(x = iteration, y = value, color = region)) +
...
for (r in regList) {
  ...
  dataMarkupPlotREMIND <- dataMarkupREMIND %>%
    filter(region %in% r[[1]]) %>%
    group_by(year, technology, iteration) %>%
  ...
  pMarkupYearFix <- ggplot() +
    ...
    geom_point(data = dataMarkupPlotPyPSA, aes(x = year, y = value, color = carrier)) +
    scale_color_manual(
      name = "Technology",
      values = colorsTech,
      labels = namesTech,
    ...
    ggtitle("Markup over years", regSubtitle(r)) +
    facet_wrap(~ iteration)
  ...
  pMarkUpIterFix <- ggplot() +
    ...
    geom_point(data = dataMarkupPlotPyPSA, aes(x = iteration, y = value, color = carrier)) +
    scale_color_manual(
      name = "Technology",
      values = colorsTech,
      labels = namesTech,
    ...
    labs(x = "Year", y = "Markup ($/MWh)") +
    ggtitle("Markup over iterations", regSubtitle(r)) +
    facet_wrap(~ year, scales = "free")
...
for (r in regList) {
  for (y in years) {
    ...
    pCapComp <- ggplot() +
      ...
      labs(x = "Iteration", y = "Capacity (GW)") +
      ggtitle(paste("Capacity over iterations, model comparison, year", y), regSubtitle(r))

...
dataPeakResLoadREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "p32_PyPSA_PeakResLoadRel",
    columns = c("ttot" = "year", "all_regi" = "region", "value" = "value"),
    colFilter = list("year" = years, "region" = regions),
    restoreZeros = FALSE)
...
pPeakResLoadYear <- ggplot() +
  geom_line(data = dataPeakResLoadREMIND, aes(x = year, y = value, color = region)) +
...
pPeakResLoadIter <- ggplot() +
  geom_line(data = dataPeakResLoadREMIND, aes(x = iteration, y = value, color = region)) +
...
Generation shares are not explicitly coupled between REMIND and PyPSA-Eur, but used to parametrise the pre-factor equations that help REMIND's convergence.
...
if (length(regions) > 1) {
  ...
  pElecPriceYear <- pElecPriceYear + facet_wrap(~ region)
}
...
if (length(regions) > 1) {
  ...
  pElecPriceIter <- pElecPriceIter + facet_wrap(~ region)
}
...
if (length(regions) > 1) {
  ...
  pMarketValueYear <- pMarketValueYear + facet_wrap(~ region)
}
...
if (length(regions) > 1) {
  ...
  pMarketValueIter <- pMarketValueIter + facet_wrap(~ region)
}

...
### Markups (generation technologies)
...
### Markups (electrolysis)

```{r markup electrolysis}

# REMIND
dataMarkupElectrolysisREMIND <- readREMINDgdx(
    paths = rmFiles,
    ...
    restoreZeros = FALSE,
    recalcUnit = trUSDpTWa2USDpMWh) %>%
  revalue.levels(technology = c("elh2" = "electrolysis"))

# PyPSA electricity price for electrolysis
dataMarkupElectrolysisPyPSA <- readPyPSAcsv(
...
# Plot
pYear <- ggplot() +
  geom_line(data = dataMarkupElectrolysisREMIND, aes(x = year, y = value, color = region)) +
  geom_point(data = dataMarkupElectrolysisPyPSA, aes(x = year, y = value, color = region)) +
  labs(x = "Year", y = "Markup ($/MWh)") +
...
pIter <- ggplot() +
  geom_line(data = dataMarkupElectrolysisREMIND, aes(x = iteration, y = value, color = region)) +
  geom_point(data = dataMarkupElectrolysisPyPSA, aes(x = iteration, y = value, color = region)) +
  labs(x = "Iteration", y = "Markup ($/MWh)") +
  scale_x_continuous(breaks = integer_breaks()) +
...
pMarkUpRevenue <- ggplot() +
    geom_line(data = dataMarkUpRevenue, aes(x = year, y = value)) +
    labs(x = "Year", y = "Markup revenue (Million $/yr)") +
...
dataCapPyPSA <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "optimal_capacities.csv")) %>%
  rename(carrier = general_carrier) %>%
  filter(carrier %in% names(py2genTech)) %>%
  # Convert MW to GW and MWh to GWh
  mutate(value = value / 1e3) %>%
  select(-component) %>%
  rename(cap = value)
  ...
  for (r in regList) {
    for (i in rmIterations){
      ...
      dataWithdrawalREMINDploti <- dataWithdrawalREMIND %>%
        ...
        group_by(year, technology) %>%
        summarise(value = sum(value)) %>% 
        order.levels(technology = c("line_losses", "battery_charger", "electrolysis_stor", "electrolysis_add", "load"))
      ...
      pGenComp <- ggplot() +
        ...
        scale_fill_manual(
          name = "Withdrawal",
          values = c(
            colorsTech[c("load", "line_losses", "battery_charger")],
            c("electrolysis_stor" = "#b08fd1",
              "electrolysis_add" = "#633a8e")),
          labels = c(
            namesTech[c("load", "line_losses", "battery_charger")],
            c("electrolysis_stor" = "Electrolysis storage",
              "electrolysis_add" = "Electrolysis end-use")),
          breaks = c("load", "electrolysis_stor", "electrolysis_add", "battery_charger", "line_losses"),
...
### Market values (demand side)

```{r end-use electricity prices}

dataElectrolysisPrice <- readPyPSAcsv(
...
p <- ggplot() +
  geom_line(data = dataElectrolysisPrice, aes(x = year, y = value, color = carrier)) +
  geom_line(data = dataLoadPrice, aes(x = year, y = value, color = carrier)) +
  scale_color_manual(
    labels = namesTech[c("electrolysis", "load")],
    values = colorsTech[c("electrolysis", "load")]
  ) +
  labs(x = "Year", y = "Electricity price ($/MWh)") +
...
pIter <- ggplot() +
  geom_line(data = dataElectrolysisPrice, aes(x = iteration, y = value, color = carrier)) +
  geom_line(data = dataLoadPrice, aes(x = iteration, y = value, color = carrier)) +
  scale_color_manual(
    labels = namesTech[c("electrolysis", "load")],
    values = colorsTech[c("electrolysis", "load")]
  ) +
...
dataHourlyElecPricesPyPSA <- readPyPSAcsv(
  ...
  filter(carrier == "AC",
         component == "Load") %>%
  select(-c("carrier", "component")) %>%
  pivot_longer(cols = -c("year", "region", "iteration"),
               names_to = "time",
               values_to = "value") %>%
  group_by(year, region, iteration) %>%
  mutate(hour = 1:n()) %>%
...
# Simple time average
dataTimeAverage <- dataHourlyElecPricesPyPSA %>%
  group_by(year, region) %>%
  summarise(value = mean(value))

# Load-weighted average
dataLoadPrices <- readPyPSAcsv(
  paths = file.path(pyReportingDirs, "load_prices.csv")) %>%
  rename(carrier = general_carrier) %>%
  filter(carrier == "AC") %>%
  select(-carrier) %>%
...
pPriceDuration <- ggplot() +
  ...
  geom_hline(data = dataTimeAverage, aes(yintercept = value, color = "Time average")) +
  geom_hline(data = dataLoadPrices, aes(yintercept = value, color = "Load-weighted average")) +
...
pPriceDurationLog <- ggplot() +
  ...
  scale_color_discrete("Average") +
  labs(x = "Hours of year (fraction)", y = "Electricity price ($/MWh)") +
  ggtitle(paste("Price duration curve, log scale, iteration", maxiter)) +
  scale_y_continuous(trans = "log10") +
  facet_wrap(~ year)
...
for iteration in df['iteration'].unique():
    ...
    for _, row in df[df['iteration'] == iteration].iterrows():
    ...
    fig.suptitle(f"Iteration {iteration}", fontsize=10, y=0.99)
    plt.show();
    plt.close(fig);
...
for iteration in df['iteration'].unique():
    ...
    for _, row in df[df['iteration'] == iteration].iterrows():
        ...
        p_negative = p_week.where(p_week < 0).dropna(how="all", axis=1).drop(columns='Load')

        # Get load data
        p_load = p_week['Load'].abs()
    ...
    fig.suptitle(f"Generation mix in summer, Iteration {iteration}", fontsize=10, y=0.98);
    plt.show();
    plt.close(fig);
...
for iteration in df['iteration'].unique():
    ...
    fig.legend(handles, labels, loc='center left', bbox_to_anchor=(0.87, 0.5), ncol=1);
    plt.tight_layout(rect=[0, 0, 0.85, 0.95]);  # Adjust the rect parameter to make space for the legend
    fig.suptitle(f"Generation mix in winter, Iteration {iteration}", fontsize=10, y=0.98);
    plt.show();
    plt.close(fig);
...
```{python residual load duration curve, echo=FALSE, warning = FALSE, message = FALSE, eval=FALSE}
...
for iteration in df['iteration'].unique():
    ...
    for _, row in df[df['iteration'] == iteration].iterrows():
        ...
        # Get total load sorted
        ldc = energy_balance['Load'].abs().sort_values(ascending=False)

        # List of generation technologies to subtract
        generation_technologies = (
            ['solar', 'onwind', 'offwind-ac', 'offwind-dc', 'ror',
            'hydro', 'PHS', 'battery discharger', 'oil', 'CCGT', 'OCGT',
            'biomass', 'lignite', 'coal', 'nuclear', 'H2 fuel cell',
            'H2 electrolysis', 'battery charger']
            )

        # Initialize the residual load with the total load
        residual_load = energy_balance['Load'].abs()

        # Dictionary to store the sorted residual loads
        sorted_residual_loads = {}

        # Cumulatively subtract each generation technology and sort the residual load
        for tech in generation_technologies:
            try:
                residual_load -= energy_balance[tech]
                sorted_residual_loads[tech] = residual_load.sort_values(ascending=False)
            except KeyError:
                continue
        ...
        # To speed up the plotting and make the plot smaller, only show every 10th value
        ldc = ldc.iloc[::10]
        for tech in sorted_residual_loads:
            sorted_residual_loads[tech] = sorted_residual_loads[tech].iloc[::10]

        # Initialize the previous residual load for filling
        previous_residual_load = ldc

        # Plot the residual load duration curve
        for tech, sorted_residual_load in sorted_residual_loads.items():
            ax.fill_between(range(len(sorted_residual_load)),
                            sorted_residual_load.values,
                            previous_residual_load.values,
                            label=f'{tech}',
                            color=color_series[tech]);
            previous_residual_load = sorted_residual_load
        
        # Plot load
        ax.plot(ldc.values, label='Load', color = 'black');
        ax.set_title(f"Year: {row['year']}")

        # Update global y-axis limits
        global_min = min(global_min, min([sorted_residual_load.min() for sorted_residual_load in sorted_residual_loads.values()]))
        global_max = max(global_max, ldc.max())

        # Only show y axis labels for the last row
        if counter < 10:
            ax.set_xticks([])
            ax.set_xlabel('')
    ...
    fig.legend(handles, labels, loc='center left', bbox_to_anchor=(0.87, 0.5), ncol=1)
    plt.tight_layout(rect=[0, 0, 0.85, 0.95])  # Adjust the rect parameter to make space for the legend
    fig.suptitle(f"Residual load duration curve, Iteration {iteration}", fontsize=10, y=0.98)
    plt.show();
    plt.close(fig);

...
pCapStorYear <- ggplot() +
  ...
  geom_point(data = dataCapStorPyPSA, aes(x = year, y = value, color = carrier)) +
  scale_color_manual(name = "Technology",
                     values = colorsTech,
                     labels = namesTech) +
  labs(x = "Year", y = "Capacity (GW)") +
...
pCapStorIter <- ggplot() +
  ...
  geom_point(data = dataCapStorPyPSA, aes(x = iteration, y = value, color = carrier)) +
  scale_color_manual(name = "Technology",
                     values = colorsTech,
                     labels = namesTech) +
  labs(x = "Iteration", y = "Capacity (GW)") +
...
dataCapPyPSA <- readPyPSAcsv(
  ...
  select(!type) %>%
  rename(technology = carrier) %>%
  # Only show technologies that are in both models
...
# REMIND hydrogen storage technologies
...
# Combine generation with fuel cell to supply
dataSupplyREMIND <- bind_rows(dataGenREMIND, dataProdSe %>% filter(technology == "fuel_cell"))
...
dataWithdrawalREMIND <- bind_rows(dataLoadREMIND, dataProdSe %>% filter(technology == "electrolysis")) %>%
  mutate(value = -value)
  ...
  for (r in regList) {
    for (i in rmIterations){
      ...
      pGenComp <- ggplot() +
        ...
        scale_fill_manual(
          name = "Withdrawal",
          values = colorsTech[c("load", "line_losses", "electrolysis", "battery_charger")],
          labels = namesTech[c("load", "line_losses", "electrolysis", "battery_charger")],
          breaks = c("load", "line_losses", "electrolysis", "battery_charger"),

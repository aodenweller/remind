...
# Special treatment for electrolysers: Costs in costs.csv are per MWel, not per MW
dataEtaPyPSA <- readPyPSAcsv(
  paths = file.path(pyResourcesDirs, "costs.csv")) %>%
  filter(parameter == "efficiency",
         technology == "electrolysis") %>%
  rename(eta_electrolysis = value) %>%
  select(iteration, year, eta_electrolysis)
...
dataCapexPyPSArm <- dataCapexPyPSA %>%
  ...
  mutate(value = if_else(technology == "electrolysis", value / eta_electrolysis, value)) %>%
...
### Specific capital costs w/ adjustment costs (store)
...
dataCapexREMIND <- readREMINDgdx(
    ...
    recalcUnit = trUSDpTW2USDpMW) %>%
  filter(iteration %in% pyIterations) %>%
  revalue.levels(technology = c("h2stor" = "hydrogen storage underground"))

# PyPSA-Eur
dataCapexPyPSA <- readPyPSAcsv(
  paths = file.path(pyResourcesDirs, "costs.csv")) %>%
  filter(parameter == "investment",
         technology %in% c("hydrogen storage underground"))
...
dataCapREMIND <- readREMINDgdx(
    ...
    restoreZeros = FALSE) %>%
  filter(!(technology %in% c("elh2", "h2turb", "h2stor"))) %>%
  select(-grade)
...
dataPreInvCapREMIND <- readREMINDgdx(
    ...
    restoreZeros = TRUE,
    recalcUnit = 1e3) %>%
  mutate(p32_preInvCap = ifelse(technology == "h2stor", p32_preInvCap / 1e3, p32_preInvCap))
...
dataPreInvCapAvgREMIND <- readREMINDgdx(
    ...
    restoreZeros = TRUE,
    recalcUnit = 1e3) %>%
  mutate(p32_preInvCapAvg = ifelse(technology == "h2stor", p32_preInvCapAvg / 1e3, p32_preInvCapAvg)) %>%
  revalue.levels(technology = rm2genTech) %>%
  group_by(year, region, technology, iteration) %>%
...
dataCapREMIND <- readREMINDgdx(
    ...
    restoreZeros = FALSE,
    recalcUnit = 1e3) %>%   # TW to GW
  mutate(vm_cap = ifelse(technology == "h2stor", vm_cap / 1e3, vm_cap)) %>%
  select(-grade)  # Remove grade column
...
dataPreInvCapPyPSA <- readPyPSAcsv(
  paths = file.path(pyValidationDirs, "preinstalled_capacities.csv")) %>%
  # Convert MW to GW and MWh to TWh
  mutate(value = ifelse(component == "Store", value / 1e6, value / 1e3)) %>%
  rename(preInvCap = value)
...
dataCapPyPSA <- readPyPSAcsv(
  ...
  # Convert MW to GW and MWh to TWh
  mutate(value = ifelse(carrier == "H2", value / 1e6, value / 1e3)) %>%
  select(-type) %>%
  rename(cap = value)
...
dataCapREMINDplot <- dataCapREMIND %>%
  revalue.levels(technology = rm2genTech) %>%
  filter(!(technology %in% c("electrolysis", "fuel_cell", "hydrogen_storage"))) %>%
  group_by(year, region, technology, iteration) %>%

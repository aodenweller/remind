...
#  function(x, options) sprintf('{\\color{%s}%s\\color{black}}',
#                               color, x)
#}
color_block = function(color) {
...
dataMarkupREMIND <- readREMINDgdx(
  ...
  # Markups are read at the end of iteration 1 after the first PyPSA-Eur run,
  # but only take effect in the next iteration
  mutate(iteration = iteration + 1)
...
dataMarkUpRevenue <- readREMINDgdx(
    ...
    colFilter = list("year" = years, "region" = regions),
    restoreZeros = FALSE,
    recalcUnit = -1E-6)  # T$/yr to Million $/yr
...
### Pre-investment capacities and capacity expansion

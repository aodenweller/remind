...
# REMIND directories
rmOutdir <- getwd()
#rmOutdir <- "/p/tmp/adrianod/remind-pypsa/output/PyPSA_base_testOneRegi_2023-07-17_16.36.56"
...
readPyPSAcsv <- function(paths) {
  ...
  for (p in paths) {
    ...
    iteration <- as.integer(str_extract(p, "(?<=i)\\d{1,}(?=/)"))
    # Read data
...
readGDX <- function(rmFile, gdxVar, columns, colFilter,
  ...
  # If no data is found create tibble with dimensions from colFilter and values zero
  if (nrow(data) == 0) {
    data <- tibble::tibble(expand.grid(colFilter)) %>% 
      mutate(value = 0)
  } else {
     data <- data %>% 
...
readREMINDgdx <- function(paths, gdxVar, columns, colFilter,
  ...
  for (p in paths) {
    ...
    iteration <- as.integer(str_extract(p, "(?<=(fulldata_)|(REMIND2PyPSAEUR_))\\d{1,}(?=.gdx)"))
...
vmCapDistr <- readREMINDgdx(
  paths = rmFulldataFiles,
  gdxVar = "vm_capDistr",
  columns = c("tall" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "vmCapDistr"),
  colFilter = list("year" = years, "region" = regions, "technology" = names(rm2genTech)),
  restoreZeros = FALSE)
...
dataCapREMIND <- readREMINDgdx(
  ...
  colFilter = list("year" = years, "region" = regions, "technology" = names(rm2pyTech), "grade" = 1),
  restoreZeros = FALSE,
...
dataCapPyPSA <- readPyPSAcsv(
  ...
  mutate(iteration = iteration + 1,  # Increment iteration
         value = value / 1e3) %>%  # Convert capacities to GW
  filter(!is.na(value),
         value > 0.1,

...
```{r specific capital costs, echo = FALSE, message = FALSE}
...
### Fixed O&M costs
```{r fixed o&m costs, echo = FALSE, message = FALSE}
...
### Lifetimes
```{r lifetimes, echo = FALSE, message = FALSE}
...
### Discount rates
```{r discount rates, echo = FALSE, message = FALSE}
...
### Variable O&M costs
```{r variable o&m costs, echo = FALSE, message = FALSE}
...
### Fuel costs
```{r fuel costs, echo = FALSE, message = FALSE}
...
### Efficiencies
```{r efficiencies, echo = FALSE, message = FALSE}
...
### CO2 intensity
```{r co2 intensity, echo = FALSE, message = FALSE}
...
```{r capacities, echo = FALSE, message = FALSE, fig.width = 10, fig.height = 12}
...
dataCapREMINDplot <- dataCapREMIND %>% 
  quitte::revalue.levels(technology = rm2genTech) %>%
  quitte::revalue.levels(technology = namesTech) %>%
  group_by(.data$year, .data$region, .data$technology, .data$iteration) %>%
...
dataCapPyPSAplot <- dataCapPyPSA %>% 
  ...
  quitte::revalue.levels(carrier = py2genTech) %>% 
  quitte::revalue.levels(carrier = namesTech) %>%
  group_by(.data$year, .data$carrier, .data$iteration) %>%
  summarise(value = sum(.data$value)) %>%
  mutate(technology = as.factor(carrier))

pCap <- ggplot() +
  geom_bar(data = dataCapREMINDplot,
           mapping = aes(x = year, y = value, fill = "REMIND"),
           stat = "identity",
           position = position_nudge(x = -0.5),
           width = 1) +
  geom_bar(data = dataCapPyPSAplot,
  ...
  labs(x = "Year", y = "Capacity (GW)") +
  ggtitle("Capacity over years, model comparison") +
  facet_grid(technology ~ iteration, scales = "free")

print(pCap)

# Plot REMIND capacities stacked by technology
dataCapREMINDplot <- dataCapREMIND %>% 
  quitte::revalue.levels(technology = rm2genTech) %>%
  group_by(.data$year, .data$region, .data$technology, .data$iteration) %>%
  summarise(value = sum(.data$value)) %>% 
  mutate(technology = factor(technology, levels = names(namesTech)))
...
pCapREMIND <- ggplot() + 
  ...
  labs(x = "Year", y = "Capacity (GW)") +
  ggtitle("Capacity over years, REMIND") +
  facet_wrap(~ iteration, scales = "free")

print(pCapREMIND)

# Plot PyPSA capacities stacked by technology
dataCapPyPSAplot <- dataCapPyPSA %>% 
  filter(type == "Generator") %>% 
  quitte::revalue.levels(carrier = py2genTech) %>% 
  group_by(.data$year, .data$carrier, .data$iteration) %>%
  summarise(value = sum(.data$value)) %>%
  mutate(technology = factor(carrier, levels = names(namesTech)))
...
pCapPyPSA <- ggplot() + 
  ...
  labs(x = "Year", y = "Capacity (GW)") +
  ggtitle("Capacity over years, PyPSA-Eur") +
  facet_wrap(~ iteration, scales = "free")
...
```{r capacity comparison, echo = FALSE, message = FALSE, fig.width = 10, fig.height = 6}
...
```{r generation, echo = FALSE, message = FALSE, fig.width = 10, fig.height = 12}

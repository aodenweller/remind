...
# Helper function to format the ggplot subtitle
regSubtitle <- function(r) {
  region <- gsub('[c()"]| \\[\\]', "", r)
  region <- gsub(',', " +", region)
  paste("Region(s):", gsub('[c()"]| \\[\\]', "", r))
}
...
twa2mwh <- 8760 * 1e6
tC2tCO2 <- 44 / 12
#GtCpTWa2GtCO2pMWh <- 1E12 / twa2mwh * tC2tCO2
...
# Function to extract unique values with a given tolerance
...
for (r in regList){
  ...
  dataPreInvCapREMINDplot <- dataPreInvCapREMIND %>%
    filter(region %in% r[[1]]) %>%
    group_by(.data$year, .data$technology, .data$iteration) %>%
  ...
  pYearFix <- ggplot() +
    ...
    geom_point(data = dataPreInvCapPyPSAplot, aes(x = year, y = preInvCap, color = carrier)) +
  ...
  pIterationFix <- ggplot() +
    geom_line(data = dataPreInvCapREMINDplot, aes(x = iteration, y = preInvCap, color = technology)) +
    geom_point(data = dataPreInvCapPyPSAplot, aes(x = iteration, y = preInvCap, color = carrier)) +
    scale_color_manual(name = "Technology", values = colorsTech, labels = namesTech) +
...
dataCO2intWeightedREMIND <- dataCO2intREMIND %>%
  left_join(dataGenTechREMIND, by = c("technology")) %>%
  revalue.levels(technology = rm2pyTech) %>%
  group_by(year, technology, iteration) %>%
...
TODO: Which weights to use for CO2 prices?

...
knitr::opts_chunk$set(dpi = 300, fig.pos = "p", out.extra = "")
theme_set(theme_bw())
...
```{r, capacity investment, echo = FALSE, message = FALSE}
...
dataPreInvCapPyPSA <- readPyPSAcsv(
  ...
  summarise(preInvCap = unique(preInvCap))  # ToDo: Technology mapping in PyPSA?
...
```{r capacity comparison, echo = FALSE, message = FALSE}
...
dataCapPyPSAplot <- dataCapPyPSA %>%
  filter(type == "Generator") %>%
  quitte::revalue.levels(carrier = pygen2genTech) %>%
...
```{r generation comparison, echo = FALSE, message = FALSE}

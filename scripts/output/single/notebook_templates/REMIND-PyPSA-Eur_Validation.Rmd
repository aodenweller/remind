...
rm2genTech <- c(rm2genTech, c("elh2" = "electrolysis", "h2turb" = "fuel_cell"))
...
pYear <- ggplot() +
  geom_point(data = dataElecPriceElectrolysisPyPSA,
             aes(x = year, y = value, color = region)) +
  ggtitle("Electricity prices for electrolysis over years") +
  xlab("Year") +
  scale_y_continuous(name = "USD/MWh",
                     limits = c(0, NA)) +
  facet_wrap(~ iteration, scales = "free")
...
pIter <- ggplot() +
  ...
  ggtitle("Electricity prices for electrolysis over iterations") +
  xlab("Iteration") +
  scale_y_continuous(name = "USD/MWh",
                     limits = c(0, NA)) +
  scale_x_continuous(breaks = integer_breaks()) +
...
### Hydrogen storage: Optimal capacities

For electrolysers, we import minimum capacities from PyPSA (for seasonal storage).
For fuel cells/hydrogen turbines, we import fixed capacities from PyPSA (re-electriciation).

```{r hydrogen storage capacities}
# REMIND capacities
dataCapStorREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "vm_cap",
    columns = c("tall" = "year", "all_regi" = "region", "all_te" = "technology", "rlf" = "grade", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "technology" = c("elh2", "h2turb"), "grade" = 1),
    restoreZeros = FALSE,
    recalcUnit = 1E3) %>%
  revalue.levels(technology = c("elh2" = "electrolysis", "h2turb" = "fuel_cell")) %>%
  select(-grade)
...
dataCapStorPyPSA <- dataCapStorPyPSA %>%
  left_join(dataEtaPyPSA, by = c("year", "technology", "iteration")) %>%
  mutate(value = value * eta)

# Plot by year
pCapStorYear <- ggplot() +
  geom_line(data = dataCapStorREMIND, aes(x = year, y = value, color = technology)) +
  geom_point(data = dataCapStorPyPSA, aes(x = year, y = value, color = technology)) +
  scale_color_manual(name = "Technology",
                     values = colorsTech,
                     labels = namesTech) +
  labs(x = "Year", y = "Output capacity (GW)") +
  ggtitle("Hydrogen storage capacities over years") +
  facet_wrap(~ iteration)
...
# Plot by iteration
pCapStorIter <- ggplot() +
  geom_line(data = dataCapStorREMIND, aes(x = iteration, y = value, color = technology)) +
  geom_point(data = dataCapStorPyPSA, aes(x = iteration, y = value, color = technology)) +
  scale_color_manual(name = "Technology",
                     values = colorsTech,
                     labels = namesTech) +
  labs(x = "Iteration", y = "Output capacity (GW)") +
  ggtitle("Hydrogen storage capacities over iterations") +
  facet_wrap(~ year)
...
dataCapREMINDplot <- dataCapREMIND %>%
  revalue.levels(technology = rm2genTech) %>%
  filter(!(technology %in% c("electrolysis", "fuel_cell"))) %>%
  group_by(year, region, technology, iteration) %>%
...
Don't evaluate due to unresolved issues for now.

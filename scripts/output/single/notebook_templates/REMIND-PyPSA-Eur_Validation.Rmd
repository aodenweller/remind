...
# REMIND additional hydrogen load data
dataH2DemandREMIND <- readREMINDgdx(
    paths = rm2pyFiles,
    gdxVar = "p32_ElecH2Demand",
    columns = c("ttot" = "year", "all_regi" = "region", "value" = "value"),
    colFilter = list("year" = years, "region" = regions),
    restoreZeros = FALSE,
    recalcUnit = twa2twh) %>%
  filter(iteration %in% pyIterations) %>%
  mutate(sector = "electrolysis")

# Also get electrolysis efficiency to calculate electricity demand for hydrogen
dataEtaREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "pm_eta_conv",
...
dataElectrolysisLoadREMIND <- dataH2DemandREMIND %>%
  left_join(dataEtaREMIND, by = c("year", "region", "iteration")) %>%
  mutate(value = value / eta) %>%  # Convert hydrogen demand to electricity load
  select(-eta) %>%
  mutate(sector = "electrolysis")  # Rename sector to electrolysis

dataLoadREMIND <- dataLoadREMIND %>%
  bind_rows(dataElectrolysisLoadREMIND) %>%
...
dataLoadPyPSA <- readPyPSAcsv(
  ...
  filter(component %in% c("Load", "Link"),
         general_carrier %in% c("AC", "BEV charger", "heat pump", "resistive heating")) %>%
...
dataElectrolysisLoadPyPSA <- dataLoadH2PyPSA %>%
  ...
  mutate(value = value / eta) %>%  # Convert hydrogen demand to electricity load
  select(-eta) %>%
  mutate(general_carrier = "electrolysis")  # Rename general_carrier to electrolysis
...
# By sector (without electrolysis)
pYear <- ggplot() + 
    ...
    geom_col(data = dataLoadPyPSA,
             aes(x = year, y = value, fill = general_carrier), 
             position = position_stacknudge(x = 1),
             width = 1) +
...
pIter <- ggplot() + 
    geom_col(data = dataLoadREMIND, 
             ...
             position = position_stacknudge(x = -0.15),
             width = 0.25) +
    geom_col(data = dataLoadPyPSA,
             aes(x = iteration, y = value, fill = general_carrier), 
             position = position_stacknudge(x = 0.15),
             width = 0.25) +
    scale_fill_manual(name = "Sector", values = colorsTech, labels = namesTech) +
    geom_line(data = dataLoadREMINDtot, 
...
### Additional hydrogen load

```{r additional hydrogen demand}
...
pH2DemandYear <- ggplot() +
  geom_line(data = dataH2DemandREMIND, aes(x = year, y = value, color = region)) +
  geom_point(data = dataH2DemandPyPSA, aes(x = year, y = value, color = region)) +
  labs(x = "Year", y = "Additional hydrogen demand (TWh)") +
  ggtitle("Additional hydrogen demand over years") +
  facet_wrap(~ iteration)

print(pH2DemandYear)

pH2DemandIter <- ggplot() +
  geom_line(data = dataH2DemandREMIND, aes(x = iteration, y = value, color = region)) +
  geom_point(data = dataH2DemandPyPSA, aes(x = iteration, y = value, color = region)) +
  labs(x = "Iteration", y = "Additional hydrogen demand (TWh)") +
  ggtitle("Additional hydrogen demand over iterations") +
  facet_wrap(~ year)
...
# REMIND electricity demand for electrolysis used for electricity storage
dataElectrolysisStorREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "vm_demSe",
    columns = c("ttot" = "year", "all_regi" = "region", "all_enty" = "enty1", "all_enty1" = "enty2", "all_te" = "technology", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "enty1" = "seh2", "enty2" = "seel", "technology" = "h2turb"),
...
# Need to divide dataElectrolysisStorREMIND by electrolysis efficiency
dataEtaREMIND <- readREMINDgdx(
    paths = rmFiles,
    gdxVar = "pm_eta_conv",
    columns = c("tall" = "year", "all_regi" = "region", "all_te" = "technology", "value" = "value"),
    colFilter = list("year" = years, "region" = regions, "technology" = "elh2"),
    restoreZeros = FALSE) %>%
  rename(eta_elh2 = value) %>%
  select(year, region, iteration, eta_elh2)

# Divide by efficiency
dataElectrolysisStorREMIND <- dataElectrolysisStorREMIND %>%
  left_join(dataEtaREMIND) %>%
  mutate(value = value / eta_elh2) %>%
  mutate(technology = "electrolysis_stor") %>%
  select(year, region, technology, iteration, value)

# Subtract from total electricity demand for electrolysis to get
# electricity demand for end-use sectors
dataElectrolysisAddREMIND <- dataElectrolysisREMIND %>%
  full_join(dataElectrolysisStorREMIND, by = c("year", "region", "iteration")) %>%
...
dataWithdrawalREMIND <- bind_rows(dataLoadREMIND, dataElectrolysisStorREMIND) %>%
  bind_rows(dataElectrolysisAddREMIND) %>%
...
withdrawal <- c("line_losses", "battery_charger", "electrolysis_stor", "electrolysis_add", "resistive", "heatpump", "EVs", "load")
...
color_series['resistive heating'] = "#B86B1F"  # Red for resistive heating
color_series['Load'] = "#4B5C67"
color_series['heat pump'] = "#F5835A"
color_series["BEV charger"] = "#E8E88A"
...
### State of charge (EV battery flexibility)

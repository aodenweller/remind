...
pDiscountIter <- ggplot() +
  ...
  scale_x_continuous(breaks = pyIterations) +
  scale_y_continuous(limits = c(0, NA)) +
...
for (r in regList) {
  ...
  dataMarkupAvgPlotREMIND <- dataMarkupAvgREMIND %>%
    filter(region %in% r) %>%
    group_by(year, technology, iteration) %>%
    summarise(p32_PyPSA_MarkupAvg = mean(p32_PyPSA_MarkupAvg))  # ToDo: Incorrect aggregation
...
```{r electricity trade}
if (length(cfg$gms$c32_pypsa_trade) != 0) {
  if (cfg$gms$c32_pypsa_trade == "on") {
...
dataTradePricePyPSA <- readPyPSAcsv(
    paths = file.path(pyValidationDirs, "crossborder_prices.csv")) %>%
    # Increment iteration by 1 to match REMIND
    mutate(iteration = iteration + 1)
...
dataImportPricePyPSA <- full_join(dataTradePricePyPSA, dataTradePyPSA, by = c("year", "iteration", "from", "to")) %>%
  group_by(year, to, iteration) %>%
...
dataExportPricePyPSA <- full_join(dataTradePricePyPSA, dataTradePyPSA, by = c("year", "iteration", "from", "to")) %>%
  group_by(year, from, iteration) %>%
...
for (r in regList) {
  ...
  dataCapREMINDplot <- left_join(dataCapREMIND %>% mutate(vm_cap = 1E3*vm_cap), dataPHScapREMIND) %>%
...
ggsave("markup_year_new.png", width = 16, height = 10, units = "cm", dpi = 300, bg = "white")
